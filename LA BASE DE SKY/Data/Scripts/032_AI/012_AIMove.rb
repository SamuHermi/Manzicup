...
class Battle::AI::AIMove
  ...
  def set_up(move)
    ...
    @ai.battle.moldBreaker ||= %w[IgnoreTargetAbility
  ...
  def physicalMove?(thisType = nil)
    @move.physicalMove?(thisType)
  end

  def specialMove?(thisType = nil)
    @move.specialMove?(thisType)
  end
  ...
  def pbTarget(user)
    @move.pbTarget(user.is_a?(Battle::AI::AIBattler) ? user.battler : user)
  end
  ...
  def rough_priority(user)
    ...
    ret = Battle::ItemEffects.triggerPriorityChange(user.item, user.battler, @move, ret) if user.item_active?
    ret
  end
  ...
  def base_power
    ret = @move.power
    ret = 60 if ret == 1
    return ret unless @ai.trainer.medium_skill?

    Battle::AI::Handlers.get_base_power(function_code,
  ...
  def rough_damage
    ...
    if %w[CategoryDependsOnHigherDamagePoisonTarget
          CategoryDependsOnHigherDamageIgnoreTargetAbility].include?(function_code)
  ...
  def rough_damage_modifiers(user, target, calc_type, base_dmg, multipliers, is_critical)
    ...
    if calc_type && user.has_type?(calc_type)
      multipliers[:final_damage_multiplier] *= if user.has_active_ability?(:ADAPTABILITY)
                                                 2
                                               else
                                                 1.5
                                               end
  ...
  def rough_damage_modifiers_global_abilities(user, target, calc_type, base_dmg, multipliers, is_critical)
    if @ai.trainer.medium_skill? && ((@ai.battle.pbCheckGlobalAbility(:DARKAURA) && calc_type == :DARK) ||
         (@ai.battle.pbCheckGlobalAbility(:FAIRYAURA) && calc_type == :FAIRY))
      multipliers[:power_multiplier] *= if @ai.battle.pbCheckGlobalAbility(:AURABREAK)
                                          3 / 4.0
                                        else
                                          4 / 3.0
                                        end
    end
    return unless @ai.trainer.medium_skill?

    if @ai.battle.pbCheckGlobalAbility(:TABLETSOFRUIN) && user.ability_id != :TABLETSOFRUIN && physicalMove?(calc_type)
      multipliers[:power_multiplier] *= 3 / 4.0
    end
    if @ai.battle.pbCheckGlobalAbility(:VESSELOFRUIN) && user.ability_id != :VESSELOFRUIN && specialMove?(calc_type)
      multipliers[:power_multiplier] *= 3 / 4.0
    end
    if @ai.battle.pbCheckGlobalAbility(:SWORDOFRUIN) && user.ability_id != :SWORDOFRUIN
      ...
      elsif physicalMove?(calc_type)
        multipliers[:defense_multiplier] *= 3 / 4.0
      end
    end
    return unless @ai.battle.pbCheckGlobalAbility(:BEADSOFRUIN) && user.ability_id != :BEADSOFRUIN
    ...
    if user.ability_active?
      ...
      when :ANALYTIC
        if rough_priority(user) <= 0
          ...
          multipliers[:power_multiplier] *= 1.3 unless user_faster
        end
      when :NEUROFORCE
  ...
  def rough_damage_modifiers_held_items(user, target, calc_type, base_dmg, multipliers, is_critical)
    ...
    return unless target.item_active? && target.item && !target.item.is_berry?
  ...
  def rough_damage_modifiers_gym_badges(user, target, calc_type, base_dmg, multipliers, is_critical)
    return unless @ai.battle.internalBattle
    ...
    return unless @ai.trainer.high_skill? && target.battler.pbOwnedByPlayer?
  ...
  def rough_damage_modifiers_lingering_effects(user, target, calc_type, base_dmg, multipliers, is_critical)
    ...
    if @ai.trainer.medium_skill?
      ...
      when :Electric
        if calc_type == :ELECTRIC && user.battler.affectedByTerrain?
    ...
    if @ai.trainer.medium_skill?
      ...
      when :Sun, :HarshSun
        ...
        when :WATER
          if function_code == 'IncreasePowerInSun' && %i[Sun HarshSun].include?(user.battler.effectiveWeather)
            multipliers[:final_damage_multiplier] *= 1.5
          else
  ...
  def rough_damage_modifiers_move_effects(user, target, calc_type, base_dmg, multipliers, is_critical)
    # Minimize
    return unless @ai.trainer.medium_skill? && target.effects[PBEffects::Minimize] && @move.tramplesMinimize?
  ...
  def rough_accuracy
    ...
    if @move.is_a?(Battle::Move::OHKO)
      ret = accuracy + user.level - target.level
    ...
    modifiers[:base_accuracy] * accuracy / evasion
  end
  ...
  def apply_rough_accuracy_modifiers(user, target, calc_type, modifiers)
    ...
    if user.item_active?
      if user.item == :ZOOMLENS
        modifiers[:accuracy_multiplier] *= 1.2 if (rough_priority(user) <= 0) && target.faster_than?(user)
      else
    ...
    modifiers[:accuracy_multiplier] *= 5 / 3.0 if @ai.battle.field.effects[PBEffects::Gravity] > 0
    if @ai.trainer.medium_skill?
      modifiers[:accuracy_multiplier] *= 1.2 if user.effects[PBEffects::MicleBerry]
      modifiers[:evasion_stage] = 0 if target.effects[PBEffects::Foresight] && modifiers[:evasion_stage] > 0
      modifiers[:evasion_stage] = 0 if target.effects[PBEffects::MiracleEye] && modifiers[:evasion_stage] > 0
    end
    # "AI-specific calculations below"
    if %w[IgnoreTargetDefSpDefEvaStatStages
          IgnoreTargetStatStagesNormalEffectiveAgainstFairy].include?(function_code)
      modifiers[:evasion_stage] = 0
    end # Nihil Light
    if @ai.trainer.medium_skill? && user.effects[PBEffects::LockOn] > 0 &&
  ...
  def rough_critical_hit_stage
    ...
    [crit_stage, Battle::Move::CRITICAL_HIT_RATIOS.length - 1].min
  end
