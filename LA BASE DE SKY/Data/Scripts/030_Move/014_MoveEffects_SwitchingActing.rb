...
class Battle::Move::StartSnowstormWeatherSwitchOutUser < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    if user.wild? || !@battle.pbCanChooseNonActive?(user.index)
      ...
      unless @battle.pbCanStartWeather?(weather)
        @battle.pbDisplay(_INTL('¡Pero ha fallado!'))
        return true
      end
    end
...
class Battle::Move::SwitchOutTargetStatusMove < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    return true unless target.canBeForcedOutOfBattle?(show_message)
  ...
  def pbSwitchOutTargetEffect(user, targets, numHits, switched_battlers)
    return unless switched_battlers.empty?
    return if user.fainted? || numHits == 0

    targets.each do |b|
      next if b.fainted? || b.damageState.unaffected
      next if b.wild?
      next unless b.canBeForcedOutOfBattle?(false)
...
class Battle::Move::BindTarget < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    target.effects[PBEffects::Trapping] = if user.hasActiveItem?(:GRIPCLAW)
                                            8
                                          else
                                            5 + @battle.pbRandom(2)
                                          end
...
class Battle::Move::UsedAfterUserTakesPhysicalDamage < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    unless user.effects[PBEffects::ShellTrap]
      @battle.pbDisplay(_INTL('¡Pero ha fallado!'))
      return true
    end
...
class Battle::Move::TargetUsesItsLastUsedMoveAgain < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if targetMove && targetMove.is_a?(Battle::Move) && %w[FailsIfUserDamagedThisTurn UsedAfterUserTakesPhysicalDamage BurnAttackerBeforeUserActs].include?(targetMove.function_code) # Beak Blast
      @battle.pbDisplay(_INTL('¡Pero ha fallado!')) if show_message
      return true
    end
