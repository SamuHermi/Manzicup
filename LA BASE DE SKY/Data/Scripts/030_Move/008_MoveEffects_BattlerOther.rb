...
class Battle::Move::SleepTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    !target.pbCanSleep?(user, show_message, self)
  end
...
class Battle::Move::SleepTargetNextTurn < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    return true unless target.pbCanSleep?(user, true, self)
...
class Battle::Move::PoisonTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    !target.pbCanPoison?(user, show_message, self)
  end
...
class Battle::Move::BadPoisonTarget < Battle::Move::PoisonTarget
  ...
  def pbOverrideSuccessCheckPerHit(user, target)
    Settings::MORE_TYPE_EFFECTS && statusMove? && user.pbHasType?(:POISON)
  end
end
...
class Battle::Move::ParalyzeTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    !target.pbCanParalyze?(user, show_message, self)
  end
...
class Battle::Move::BurnTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    !target.pbCanBurn?(user, show_message, self)
  end
...
class Battle::Move::FrostbiteTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    !target.pbCanFrostbite?(user, show_message, self)
  end
...
class Battle::Move::FreezeTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    !target.pbCanFreeze?(user, show_message, self)
  end
...
class Battle::Move::FreezeTargetAlwaysHitsInHail < Battle::Move::FreezeTarget
  def pbBaseAccuracy(user, target)
    return 0 if %i[Hail Snowstorm].include?(target.effectiveWeather)
...
class Battle::Move::FreezeFlinchTarget < Battle::Move
  ...
  def pbAdditionalEffect(user, target)
    ...
    target.pbFreeze(user) if target.pbCanFreeze?(user, false, self) && @battle.pbRandom(100) < chance
    target.pbFlinch(user) if @battle.pbRandom(100) < chance
  end
end
...
class Battle::Move::GiveUserStatusToTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    unless target.pbCanInflictStatus?(user.status, user, false, self)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!')) if show_message
      return true
    end
    false
...
class Battle::Move::CureUserBurnPoisonParalysis < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    unless %i[BURN POISON PARALYSIS].include?(user.status)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!'))
      return true
    end
    false
...
class Battle::Move::ConfuseTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    !target.pbCanConfuse?(user, show_message, self)
  end
  ...
  def pbAdditionalEffect(user, target)
    return unless target.affectedByAdditionalEffects?
    return if target.damageState.substitute
    return unless target.pbCanConfuse?(user, false, self)
...
class Battle::Move::AttractTarget < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    return false if damagingMove?
    return true unless target.pbCanAttract?(user, show_message)
...
class Battle::Move::UserLosesFireType < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    return unless !user.effects[PBEffects::BurnUp] && !target.damageState.unaffected
...
class Battle::Move::UserLosesElectricType < Battle::Move
  def pbMoveFailed?(user, targets)
    unless user.pbHasType?(:ELECTRIC)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!'))
      return true
    end
    false
  ...
  def pbEffectAfterAllHits(user, target)
    return unless !user.effects[PBEffects::DoubleShock] && !target.damageState.unaffected
...
class Battle::Move::SetTargetAbilityToSimple < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.unlosableAbility? || %i[SIMPLE TRUANT].include?(target.ability_id)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!')) if show_message
      return true
    end
    false
...
class Battle::Move::SetTargetAbilityToInsomnia < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.unlosableAbility? || %i[INSOMNIA TRUANT].include?(target.ability_id)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!')) if show_message
      return true
    end
    false
...
class Battle::Move::SetUserAbilityToTargetAbility < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    ...
    if target.ungainableAbility? ||
       %i[HADRONENGINE ORICHALCUMPULSE].include?(target.ability_id)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!')) if show_message
      return true
    end
    false
...
class Battle::Move::SetTargetAbilityToUserAbility < Battle::Move
  ...
  def pbFailsAgainstTarget?(user, target, show_message)
    if target.unlosableAbility? ||
       %i[HADRONENGINE ORICHALCUMPULSE TRUANT].include?(target.ability_id)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!')) if show_message
      return true
    end
    false
...
class Battle::Move::UserTargetSwapAbilities < Battle::Move
  ...
  def pbMoveFailed?(user, targets)
    ...
    if user.ungainableAbility? ||
       %i[HADRONENGINE ORICHALCUMPULSE].include?(user.ability_id)
      @battle.pbDisplay(_INTL('¡Pero ha fallado!'))
      return true
    end
    false
...
class Battle::Move::NegateTargetAbilityIfTargetActed < Battle::Move
  def pbEffectAgainstTarget(user, target)
    ...
    return if @battle.choices[target.index][0] != :UseItem &&
              !(%i[UseMove Shift].include?(@battle.choices[target.index][0]) && target.movedThisRound?)
...
class Battle::Move::UserVulnerableUntilNextAction < Battle::Move
  def pbEffectAgainstTarget(user, target)
    user.effects[PBEffects::Vulnerable] = true unless target.damageState.unaffected
  end
end
...
class Battle::Move::HitsTargetInSkyGroundsTarget < Battle::Move
  ...
  def pbEffectAfterAllHits(user, target)
    ...
    if target.inTwoTurnAttack?('TwoTurnAttackInvulnerableInSky',
      ...
      @battle.pbClearChoice(target.index) unless target.movedThisRound?
    end
...
class Battle::Move::StartGravity < Battle::Move
  ...
  def pbEffectGeneral(user)
    ...
    @battle.allBattlers(true).each do |b|
      ...
      @battle.pbDisplay(_INTL('¡{1} no ha podido mantenerse en el aire por la gravedad!', b.pbThis)) if showMessage
    end
  end
end
