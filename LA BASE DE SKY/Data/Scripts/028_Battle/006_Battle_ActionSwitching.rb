class Battle
  ...
  def pbCanSwitch?(idxBattler, idxParty = -1, party_screen = nil)
    ...
    return false unless pbCanSwitchIn?(idxBattler, idxParty, party_screen)
    ...
    pbCanSwitchOut?(idxBattler, party_screen)
  end
  ...
  def pbPartyScreen(idxBattler, checkLaxOnly = false, canCancel = false, shouldRegister = false)
    ...
    @scene.pbPartyScreen(idxBattler, canCancel) do |idxParty, party_screen|
      if checkLaxOnly
        next false unless pbCanSwitchIn?(idxBattler, idxParty, party_screen)
  ...
  def pbSwitchInBetween(idxBattler, checkLaxOnly = false, canCancel = false)
    ...
    @battleAI.pbDefaultChooseNewEnemy(idxBattler)
  end
  ...
  def pbGetReplacementPokemonIndex(idxBattler, random = false)
    ...
    choices[pbRandom(choices.length)]
  end

  # Actually performs the recalling and sending out in all situations.
  def pbRecallAndReplace(idxBattler, idxParty, randomReplacement = false, batonPass = false)
    @scene.pbRecall(idxBattler) unless @battlers[idxBattler].fainted?
  ...
  def pbOnBattlerEnteringBattle(battler_index, skip_event_reset = false)
    ...
    battler_index = [battler_index] unless battler_index.is_a?(Array)
  ...
  def pbRecordBattlerAsParticipated(battler)
    ...
    if !battler.opposes? && %i[AMULETCOIN LUCKINCENSE].include?(battler.item_id)
