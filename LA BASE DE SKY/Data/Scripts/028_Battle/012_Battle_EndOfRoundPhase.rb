class Battle
  ...
  def pbEORUseFutureSight(position, position_index)
    ...
    return unless moveUser # User is fainted
  ...
  def pbEndOfRoundPhase
    ...
    if Settings::AFFECTION_EFFECTS && @internalBattle
      priority.each do |battler|
        ...
        when :PARALYSIS
          pbDisplay(_INTL('¡{1} reunió toda su energía para romper su parálisis para no decepcionarte!',
                          battler.pbThis))
        when :FROZEN
          pbDisplay(_INTL('¡{1} derritió el hielo con su ardiente determinación para no decepcionarte!',
                          battler.pbThis))
        end
      end
    end
    ...
    2.times do |side|
      ...
      @sides[side].effects[PBEffects::EchoedVoiceCounter] = 0 unless @sides[side].effects[PBEffects::EchoedVoiceUsed]
