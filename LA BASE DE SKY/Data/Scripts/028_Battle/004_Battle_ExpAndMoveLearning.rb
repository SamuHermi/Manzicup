class Battle
      ...
      if numPartic > 0 || expShare.length > 0 || expAll
        ...
        if expAll
          ...
          eachInTeam(0, 0) do |pkmn, i|
            ...
            if showMessage && expShare.length > numPartic && pbParty(0).length > 1
              pbDisplayPaused(_INTL('¡Tus otros Pokémon también ganaron puntos de experiencia!'))
            end
            showMessage = false
            pbGainEVsOne(i, b)
  ...
  def pbGainEVsOne(idxParty, defeatedBattler)
    ...
    unless Battle::ItemEffects.triggerEVGainModifier(pkmn.item, pkmn, evYield)
  ...
  def pbGainExpOne(idxParty, defeatedBattler, numPartic, expShare, expAll, showMessages = true)
    ...
    i = Battle::ItemEffects.triggerExpGainModifier(initialItem(0, idxParty), pkmn, exp) if i < 0
    exp = i if i >= 0
    ...
    if newLevel < curLevel
      ...
      raise _INTL('El nuevo nivel de {1} es menor que su nivel actual, lo que no debería suceder.',
                  pkmn.name) + "\n[#{debugInfo}]"
    end
    # Give Exp
