...
#===============================================================================
def pbBoxesFull?
  $player.party_full? && $PokemonStorage.full?
end
...
def pbNickname(pkmn)
  ...
  return unless pbConfirmMessage(_INTL('¿Te gustaría ponerle un mote a {1}?', species_name))
...
def pbAddPokemon(pkmn, level = 1, see_form = true)
  ...
  pkmn = Pokemon.new(pkmn, level) unless pkmn.is_a?(Pokemon)
  species_name = pkmn.speciesName
...
# Giving Pokémon to the player (will send to storage if party is full)
#===============================================================================
def pbReceivePokemon(pkmn, level = 5, see_form = true)
  pkmn = Pokemon.new(pkmn, level) unless pkmn.is_a?(Pokemon)
  species_name = pkmn.speciesName
  was_owned = $player.owned?(pkmn.species)
  $player.pokedex.set_seen(pkmn.species)
...
def pbAddForeignPokemon(pkmn, level = 1, owner_name = nil, nickname = nil, owner_gender = 0, see_form = true)
  ...
  pkmn.name = nickname[0, Pokemon::MAX_NAME_SIZE] unless nil_or_empty?(nickname)
  pkmn.calc_stats
  if owner_name
...
def pbGenerateEgg(pkmn, text = '')
  ...
  pkmn = Pokemon.new(pkmn, Settings::EGG_LEVEL) unless pkmn.is_a?(Pokemon)
  # Set egg's details
...
def pbSize(pkmn)
  ...
  ((((s - xyz[2]) / xyz[1]) + xyz[0]).floor * baseheight / 10).floor
end
