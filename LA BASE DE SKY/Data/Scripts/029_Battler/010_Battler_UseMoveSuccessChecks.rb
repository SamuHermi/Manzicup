...
class Battle::Battler
  ...
  def pbCanChooseMove?(move, commandPhase, showMessages = true, specialUsage = false)
    ...
    @effects[PBEffects::ChoiceBand] = nil unless pbHasMove?(@effects[PBEffects::ChoiceBand])
  ...
  def pbObedienceCheck?(choice)
    ...
    return true unless @battle.internalBattle
    return true unless @battle.pbOwnedByPlayer?(@index)
  ...
  def pbTryUseMove(choice, move, specialUsage, skipAccuracyCheck)
    ...
    if hasActiveAbility?(:TRUANT)
      ...
      unless @effects[PBEffects::Truant] # True means loafing, but was just inverted
    ...
    if @effects[PBEffects::Flinch]
      ...
      Battle::AbilityEffects.triggerOnFlinch(ability, self, @battle) if abilityActive?
      @lastMoveFailed = true
      return false
    end
    # Confusion
  ...
  def pbSuccessCheckAgainstTarget(move, user, target, targets)
    ...
    unless user.inTwoTurnAttack?('TwoTurnAttackInvulnerableInSkyTargetCannotAct')
      # Two-turn attacks can't fail here in the charging turn
      return true if user.effects[PBEffects::TwoTurnAttack]

      # Semi-invulnerable target
      unless pbSuccessCheckSemiInvulnerable(move, user, target)
    ...
    if move.pbFailsAgainstTarget?(user, target, show_message)
      PBDebug.log(format("[Move failed] In function code %s's def pbFailsAgainstTarget?", move.function_code))
      return false
    end
    ...
    unless user.hasActiveAbility?(:UNSEENFIST) && move.pbContactMove?(user)
      # Wide Guard
    ...
    if move.damagingMove? && move.calcType == :GROUND &&
      ...
      if (target.hasActiveAbility?(:LEVITATE) || target.hasActiveAbility?(:ORNITHES)) && !target.beingMoldBroken?
        if show_message
      ...
      if target.hasActiveItem?(:AIRBALLOON)
        if show_message
          @battle.pbDisplay(_INTL('¡{1} es inmune a los movimientos de tipo Tierra gracias a {2}!!', target.pbThis,
                                  target.itemName))
        end
        return false
      end
      if target.effects[PBEffects::MagnetRise] > 0
        if show_message
          @battle.pbDisplay(_INTL('¡{1} es inmune a los movimientos de tipo Tierra gracias a Levitón!',
                                  target.pbThis))
        end
        return false
      end
      if target.effects[PBEffects::Telekinesis] > 0
        if show_message
          @battle.pbDisplay(_INTL('¡{1} es inmune a los movimientos de tipo Tierra gracias a Telequinesis!',
                                  target.pbThis))
        end
        return false
      end
