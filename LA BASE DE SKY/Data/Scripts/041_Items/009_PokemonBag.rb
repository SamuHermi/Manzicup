...
#===============================================================================
class PokemonBag
  attr_accessor :last_viewed_pocket, :last_pocket_selections
  attr_reader :registered_items, :new_items, :favourite_items, :ready_menu_selection

  def self.pocket_names
    Settings.bag_pocket_names
  end
  ...
  def last_viewed_index(pocket)
    ...
    rearrange
    [@last_pocket_selections[pocket], @pockets[pocket].length].min || 0
  end
  ...
  def quantity(item)
    ...
    return 0 unless item_data

    pocket = item_data.pocket
    ItemStorageHelper.quantity(@pockets[pocket], item_data.id)
  end
  ...
  def add_all(item, qty = 1)
    return false unless can_add?(item, qty)
  ...
  def remove(item, qty = 1)
    ...
    pocket = item_data.pocket
    ItemStorageHelper.remove(@pockets[pocket], item_data.id, qty)
  end
  ...
  def remove_all(item, qty = 1)
    return false unless can_remove?(item, qty)
  ...
  def registered?(item)
    ...
    @registered_items.include?(item_data.id)
  end
  ...
  def register(item)
    ...
    @registered_items.push(item_data.id) unless @registered_items.include?(item_data.id)
  end
  ...
  def favourite?(item)
    ...
    @favourite_items ||= []
    @favourite_items.include?(item_data.id)
  end
  ...
  def favourite(item)
    ...
    @favourite_items ||= []
    @favourite_items.push(item_data.id) unless @favourite_items.include?(item_data.id)
  end
  ...
  def new?(item)
    ...
    @new_items ||= []
    @new_items.include?(item_data.id)
  end
  ...
  def mark_as_new(item)
    ...
    @new_items ||= []
    @new_items.push(item_data.id) unless @new_items.include?(item_data.id)
  end
  ...
  def max_pocket_size(pocket)
    Settings::BAG_MAX_POCKET_SIZE[pocket - 1] || -1
  end
...
class PCItemStorage
  ...
  # Unused
  def get_item(index)
    index < 0 || index >= @items.length ? nil : @items[index][0]
  end

  # Number of the item in the given index
  # Unused
  def get_item_count(index)
    index < 0 || index >= @items.length ? 0 : @items[index][1]
  end
  ...
  def can_add?(item, qty = 1)
    item = GameData::Item.get(item).id
    ItemStorageHelper.can_add?(@items, MAX_SIZE, MAX_PER_SLOT, item, qty)
  end

  def add(item, qty = 1)
    item = GameData::Item.get(item).id
    ItemStorageHelper.add(@items, MAX_SIZE, MAX_PER_SLOT, item, qty)
  end
