...
#
#===============================================================================
unless Settings::USE_NEW_OPTIONS_UI
  class PokemonSystem
    attr_accessor :textspeed, :battlescene, :battlestyle, :sendtoboxes, :givenicknames, :frame, :textskin, :screensize,
                  :language, :runstyle, :textinput, :vsync, :autotile_animations

    attr_reader :skip_move_learning, :main_volume, :bgmvolume, :sevolume, :pokemon_cry_volume
    ...
    def main_volume=(value)
      ...
      return unless $game_system.playing_bgs
    ...
    def update_vsync(vsync_value)
      ...
      updated_lines = lines.map do |line|
        ...
        elsif !vsync_value && line.match?(%r{//\s*"fixedFramerate":\s*\d+})
          line.sub(%r{//\s*}, '') # Uncomment the line
        else
      ...
      return unless Kernel.pbConfirmMessageSerious(message)
...
unless Settings::USE_NEW_OPTIONS_UI
  ...
  class Window_PokemonOption < Window_DrawableCommand
    ...
    def drawItem(index, _count, rect)
      ...
      # Draw option's name
      optionname = if index == @options.length
                     @is_sub_menu ? _INTL('Volver') : _INTL('Cerrar')
                   else
                     @options[index].name
                   end
      optionwidth = rect.width * OPTION_NAME_WIDTH_RATIO
      pbDrawShadowText(contents, rect.x, rect.y, optionwidth, rect.height, optionname,
                       index == sel_index ? SEL_NAME_BASE_COLOR : baseColor,
                       index == sel_index ? SEL_NAME_SHADOW_COLOR : shadowColor)
        ...
        if @options[index].values.length > 1
          ...
          @options[index].values.each do |value|
            pbDrawShadowText(contents, xpos, rect.y, optionwidth, rect.height, value,
                             ivalue == self[index] ? SEL_VALUE_BASE_COLOR : baseColor,
                             ivalue == self[index] ? SEL_VALUE_SHADOW_COLOR : shadowColor)
      ...
      when SliderOption
        value = format(' %d', @options[index].highest_value)
        sliderlength = rect.width - rect.x - optionwidth - contents.text_size(value).width
        xpos = optionwidth + rect.x
        contents.fill_rect(xpos, rect.y + SLIDER_BAR_Y_OFFSET + (rect.height / 2), sliderlength,
                           SLIDER_BAR_HEIGHT, baseColor)
        contents.fill_rect(
        ...
        xpos += (rect.width - rect.x - optionwidth) - contents.text_size(value).width
        pbDrawShadowText(contents, xpos, rect.y, optionwidth, rect.height, value,
