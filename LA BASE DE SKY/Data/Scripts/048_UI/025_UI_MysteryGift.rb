...
def pbEditMysteryGift(type, item, id = 0, giftname = '', password = nil)
  ...
  [id, type, item, giftname, password]
...
def pbManageMysteryGifts
  unless FileTest.exist?('MysteryGiftMaster.txt')
  ...
  loop do
    ...
    command = pbMessage('\\ts[]' + _INTL('Gestionar los Regalos Misteriosos (X = regalo encontrado ya en internet).'),
                        commands, -1, nil, command)
    # Gift chosen
    if command == -1 || command == commands.length - 1 # Cancel
      ...
      loop do
        ...
        when 2   # Receive
          ...
          $player.mystery_gifts.push(gift) unless replaced
...
def pbRefreshMGCommands(master, online)
  ...
  master.each do |gift|
    itemname = 'BLANK'
    itemname = if gift[2].is_a?(Pokemon)
                 gift[2].speciesName
               elsif gift[2].is_a?(Hash)
                 gift[2][:name]
               else
                 GameData::Item.get(gift[2]).name + format(' x%d', gift[1])
               end
...
def pbDownloadGiftWithoutPassword(trainer)
  ...
  gifts_without_password = pending.select do |gift|
    gift.length == 4 || (gift.length == 5 && (gift[4].nil? || gift[4].empty?))
  end
...
def pbDownloadGiftWithPassword(trainer)
  ...
  pending.each do |gift|
    next unless gift.length == 5 && gift[4] == password # gift[4] es la contraseña
...
def pbReceiveGiftAnimation(gift, trainer)
  ...
  pbMessage(_INTL('¡Se ha recibido el regalo!') + "\1") do
    Graphics.update
    sprite.update
  end
  pbMessage(_INTL('Por favor, recoge tu regalo del repartidor de cualquier Centro Pokémon.')) do
    Graphics.update
    sprite.update
  end
...
def pbMysteryGiftEncrypt(gift, master = true)
  ...
  if Settings::ENCRIPTAR_REGALOS_MISTERIOSOS_EN_MASTER || !master
    [Zlib::Deflate.deflate(Marshal.dump(gift))].pack('m')
  else
    gift.inspect
  end
end
...
def pbMysteryGiftDecrypt(gift, master = true)
  ...
  ret = if Settings::ENCRIPTAR_REGALOS_MISTERIOSOS_EN_MASTER || !master
          Marshal.load(Zlib::Inflate.inflate(gift.unpack1('m')))
        else
          eval(gift)
        end
  if ret
    ret.each do |gft|
      gft[2] = if gft[1] == 0 # Pokémon
                 # Cargamos el Pokémon
                 create_pkmn_from_hash(gft[2])
               else # Item
                 GameData::Item.get(gft[2]).id
               end
    end
  end
  ret
