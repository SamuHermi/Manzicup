...
class Window_PokemonBag < Window_DrawableCommand
  ...
  def item
    ...
    item = @filterlist ? thispocket[@filterlist[@pocket][index]] : thispocket[index]
    item ? item[0] : nil
  end

  def itemCount
    @filterlist ? @filterlist[@pocket].length + 1 : @bag.pockets[@pocket].length + 1
  end

  def itemRect(item)
    if item < 0 || item >= @item_max || item < top_item - 1 ||
      ...
      Rect.new(x, y, cursor_width, @row_height)
    end
  end
  ...
  def drawCursor(index, rect)
    ...
    pbCopyBitmap(contents, bmp, rect.x, rect.y + 2)
  end
  ...
  def drawItem(index, _count, rect)
    ...
    if index == itemCount - 1
      textpos.push([_INTL('CERRAR MOCHILA'), rect.x + CLOSE_BAG_TEXT_X_OFFSET, rect.y + CLOSE_BAG_TEXT_Y_OFFSET, :left,
      ...
      textpos.push(
        [@adapter.getDisplayName(item), rect.x + ITEM_NAME_X_OFFSET, rect.y + ITEM_NAME_Y_OFFSET, :left, baseColor,
         shadowColor]
      )
      ...
      if item_data.is_important?
        if @bag.registered?(item)
          pbDrawImagePositions(
            contents,
            [[_INTL('Graphics/UI/Bag/icon_register'), rect.x + rect.width + ICON_REGISTER_X_OFFSET,
              rect.y + ICON_REGISTER_Y_OFFSET, 0, 0, -1, 24]]
          )
          ...
          pbDrawImagePositions(
            contents,
            [[_INTL('Graphics/UI/Bag/icon_register'), rect.x + rect.width + ICON_REGISTER_X_OFFSET,
              rect.y + ICON_REGISTER_Y_OFFSET, 0, 24, -1, 24]]
          )
          showing_register_icon = true
        end
      end
      if item_data.show_quantity? && !showing_register_icon
        ...
        xQty    = rect.x + rect.width - contents.text_size(qtytext).width + ITEM_QUANTITY_X_OFFSET
  ...
  def refresh
    ...
    contents.clear
    @item_max.times do |i|
      next if i < top_item - 1 || i > top_item + page_item_max
...
class PokemonBag_Scene
  ...
  def pbStartScene(bag, choosing = false, filterproc = nil, resetpocket = true)
    ...
    @sprites['itemlist'] =
      Window_PokemonBag.new(@bag, @filterlist, lastpocket, 168, -8, 314, 40 + 32 + (ITEMSVISIBLE * 32))
  ...
  def pbEndScene
    pbFadeOutAndHide(@sprites) unless @oldsprites
    @oldsprites = nil
    dispose
  end
  ...
  def pbChooseNumber(helptext, maximum, initnum = 1)
    UIHelper.pbChooseNumber(@sprites['helpwindow'], helptext, maximum, initnum) { pbUpdate }
  end

  def pbShowCommands(helptext, commands, index = 0)
    UIHelper.pbShowCommands(@sprites['helpwindow'], helptext, commands, index) { pbUpdate }
  end

  def pbRefresh
    # Set the background image
    @sprites['background'].setBitmap(format('Graphics/UI/Bag/bg_%d', @bag.last_viewed_pocket))
    # Set the bag sprite
    fbagexists = pbResolveBitmap(format('Graphics/UI/Bag/bag_%d_f', @bag.last_viewed_pocket))
    if $player.female? && fbagexists
      @sprites['bagsprite'].setBitmap(format('Graphics/UI/Bag/bag_%d_f', @bag.last_viewed_pocket))
    else
      @sprites['bagsprite'].setBitmap(format('Graphics/UI/Bag/bag_%d', @bag.last_viewed_pocket))
    end
  ...
  def pbRefreshIndexChanged
    ...
    pbDrawTextPositions(
      overlay,
      [[PokemonBag.pocket_names[@bag.last_viewed_pocket - 1], POCKET_NAME_X, POCKET_NAME_Y, :center,
        POCKETNAMEBASECOLOR, POCKETNAMESHADOWCOLOR]]
    )
    # Draw slider arrows
    showslider = false
    if itemlist.top_row > 0
      overlay.blt(SLIDER_X, SLIDER_ARROW_TOP_Y, @sliderbitmap.bitmap,
                  Rect.new(0, SLIDER_ARROW_SRC_TOP_Y, SLIDER_ARROW_SRC_WIDTH, SLIDER_ARROW_SRC_HEIGHT))
      showslider = true
    end
    if itemlist.top_item + itemlist.page_item_max < itemlist.itemCount
      overlay.blt(SLIDER_X, SLIDER_ARROW_BOTTOM_Y, @sliderbitmap.bitmap,
                  Rect.new(0, SLIDER_ARROW_SRC_BOTTOM_Y, SLIDER_ARROW_SRC_WIDTH, SLIDER_ARROW_SRC_HEIGHT))
      showslider = true
    end
    # Draw slider box
    if showslider
      ...
      overlay.blt(SLIDER_X, y, @sliderbitmap.bitmap,
                  Rect.new(SLIDER_BAR_SRC_X, SLIDER_BAR_TOP_SRC_Y, SLIDER_ARROW_SRC_WIDTH, SLIDER_BAR_TOP_SRC_HEIGHT))
      i = 0
      while i * SLIDER_BAR_FILL_CHUNK_HEIGHT < boxheight - SLIDER_BAR_TOP_SRC_HEIGHT - SLIDER_BAR_BOTTOM_SRC_HEIGHT
        height = [
          boxheight - SLIDER_BAR_TOP_SRC_HEIGHT - SLIDER_BAR_BOTTOM_SRC_HEIGHT - (i * SLIDER_BAR_FILL_CHUNK_HEIGHT), SLIDER_BAR_FILL_CHUNK_HEIGHT
        ].min
        overlay.blt(SLIDER_X, y + SLIDER_BAR_TOP_SRC_HEIGHT + (i * SLIDER_BAR_FILL_CHUNK_HEIGHT), @sliderbitmap.bitmap,
                    Rect.new(SLIDER_BAR_SRC_X, SLIDER_BAR_FILL_SRC_Y, SLIDER_ARROW_SRC_WIDTH, height))
        i += 1
      end
      overlay.blt(SLIDER_X, y + boxheight - SLIDER_BAR_BOTTOM_SRC_HEIGHT, @sliderbitmap.bitmap,
                  Rect.new(SLIDER_BAR_SRC_X, SLIDER_BAR_BOTTOM_SRC_Y, SLIDER_ARROW_SRC_WIDTH, SLIDER_BAR_BOTTOM_SRC_HEIGHT))
    end
  ...
  def pbChooseItem
    ...
    pbActivateWindow(@sprites, 'itemlist') do
      loop do
        ...
        elsif Input.trigger?(Input::LEFT) # Change pockets
...
class PokemonBagScreen
  ...
  def pbStartScreen
    ...
    loop do
      ...
      if ItemHandlers.hasOutHandler(item) || (itm.is_machine? && $player.party.length > 0)
        commands[cmdUse = commands.length] = if ItemHandlers.hasUseText(item)
                                               ItemHandlers.getUseText(item)
                                             else
                                               _INTL('Usar')
                                             end
      end
  ...
  def pbWithdrawItemScreen
    $PokemonGlobal.pcItemStorage = PCItemStorage.new unless $PokemonGlobal.pcItemStorage
    ...
    loop do
      ...
      qty = @scene.pbChooseNumber(_INTL('¿Cuántos quieres tirar?'), qty) if qty > 1 && !itm.is_important?
      next if qty <= 0

      if @bag.can_add?(item, qty)
        raise 'No se pueden eliminar objetos del almacenamiento' unless storage.remove(item, qty)
        raise 'No se pueden tirar objetos del almacenamiento' unless @bag.add(item, qty)
  ...
  def pbDepositItemScreen
    ...
    loop do
      ...
      itm = GameData::Item.get(item)
      qty = @bag.quantity(item)
      qty = @scene.pbChooseNumber(_INTL('¿Cuántos quieres dejar?'), qty) if qty > 1 && !itm.is_important?
      if qty > 0
        if storage.can_add?(item, qty)
          raise 'No se pueden eliminar objetos de la Mochila' unless @bag.remove(item, qty)
          raise 'No se pueden dejar objetos en el almacenamiento' unless storage.add(item, qty)
  ...
  def pbTossItemScreen
    ...
    loop do
      ...
      qty = @scene.pbChooseNumber(_INTL('¿Cuántos {1} tirar?', itemnameplural), qty) if qty > 1
      next if qty <= 0

      itemname = itemnameplural if qty > 1
      next unless pbConfirm(_INTL('¿Quieres tirar {1} {2}?', qty, itemname))
      raise 'No se pueden eliminar objetos del almacenamiento' unless storage.remove(item, qty)
