...
class PokemonPokedexInfo_Scene
  ...
  def pbUpdate
    if @page == 2
      ...
      intensity = if intensity_time >= 0.5
                    lerp(64, 256 + 64, 0.5, intensity_time - 0.5)
                  else
                    lerp(256 + 64, 64, 0.5, intensity_time)
                  end
  ...
  def pbGetAvailableForms
    ...
    gender_differences = (GameData::Species.front_sprite_filename(@species,
                                                                  0) != GameData::Species.front_sprite_filename(
                                                                    @species, 0, 1
                                                                  ))
    ...
    ret.each do |entry|
      if entry[0] # Alternate forms, and form 0 if no gender differences
        ...
        entry[0] = case entry[1]
                   when 0 then _INTL('Macho')
                   when 1 then _INTL('Hembra')
                   else
                     multiple_forms ? _INTL('Forma Normal') : _INTL('Sin género')
                   end
      end
  ...
  def drawPageInfo
    ...
    if @show_battled_count
      ...
      textpos.push([$player.pokedex.battled_count(@species).to_s, coords[:battled_num][0], coords[:battled_num][1],
                    :left, base, shadow])
    else
    ...
    if $player.owned?(@species)
      # Write the category
      textpos.push([_INTL('Pokémon {1}', species_data.category), coords[:category][0], coords[:category][1], :left,
                    base, shadow])
      ...
      if true # @show_battled_count
        ...
        if System.user_language[3..4] == 'US' # If the user is in the United States
          ...
          textpos.push([_ISPRINTF("{1:d}'{2:02d}\"", inches / 12, inches % 12), coords[:height_num_us][0],
                        coords[:height_num_us][1], :right, base, shadow])
          textpos.push([_ISPRINTF('{1:4.1f} lbs.', pounds / 10.0), coords[:weight_num_us][0],
                        coords[:weight_num_us][1], :right, base, shadow])
        else
          textpos.push([_ISPRINTF('{1:.1f} m', height / 10.0), coords[:height_num][0], coords[:height_num][1], :right,
                        base, shadow])
          textpos.push([_ISPRINTF('{1:.1f} kg', weight / 10.0), coords[:weight_num][0], coords[:weight_num][1], :right,
                        base, shadow])
        end
      end
      ...
      species_data.types.each_with_index do |type, i|
        ...
        overlay.blt(coords[:type_icon][0] + (coords[:type_icon][1] * i), coords[:type_icon][2], @typebitmap.bitmap,
                    type_rect)
      end
    else
      # Write the category
      ...
      # Write the height and weight
      unless @show_battled_count
        if System.user_language[3..4] == 'US' # If the user is in the United States
          textpos.push([_INTL("???'??\""), coords[:height_num_us][0], coords[:height_num_us][1], :right, base, shadow])
          textpos.push([_INTL('????.? lbs.'), coords[:weight_num_us][0], coords[:weight_num_us][1], :right, base,
                        shadow])
        else
  ...
  def pbGetEncounterPoints
    ...
    GameData::Encounter.each_of_version($PokemonGlobal.encounter_version) do |enc_data|
      next unless pbFindEncounter(enc_data.types, @species) # Species isn't in encounter table
      ...
      map_width.times do |i|
        map_height.times do |j|
          ...
          next unless visible_points.include?([mappos[1] + i, mappos[2] + j]) # Point isn't visible
  ...
  def drawPageArea
    ...
    if points.length == 0
      ...
      textpos.push([_INTL('Área desconocida'), Graphics.width / 2,
                    Graphics.height / 2 + coords[:unknown_area_y_offset], :center, base, shadow])
    end
    textpos.push([@mapdata&.name, coords[:map_name][0], coords[:map_name][1], :center, base, shadow]) if @mapdata
  ...
  def drawPageForms
    ...
    textpos = [
      [GameData::Species.get(@species).name, Graphics.width / 2, Graphics.height + coords[:species_name_y_offset],
       :center, base, shadow],
...
class PokemonPokedexInfoScreen
  ...
  def pbStartScreen(dexlist, index, region, page = 1)
    ...
    ret = @scene.pbScene
    @scene.pbEndScene
    ret # Index of last species viewed in dexlist
  end
