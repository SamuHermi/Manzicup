...
module Battle::ItemEffects
  ...
  def self.triggerSpeedCalc(item, battler, mult)
    trigger(SpeedCalc, item, battler, mult, ret: mult)
  end

  def self.triggerWeightCalc(item, battler, w)
    trigger(WeightCalc, item, battler, w, ret: w)
  end

  #-----------------------------------------------------------------------------

  def self.triggerHPHeal(item, battler, battle, forced)
    trigger(HPHeal, item, battler, battle, forced)
  end

  #-----------------------------------------------------------------------------

  def self.triggerStatusCure(item, battler, battle, forced)
    trigger(StatusCure, item, battler, battle, forced)
  end

  #-----------------------------------------------------------------------------

  def self.triggerStatLossImmunity(item, battler, stat, battle, show_messages)
    trigger(StatLossImmunity, item, battler, stat, battle, show_messages)
  end

  def self.triggerOnStatLoss(item, user, move_user, battle)
    trigger(OnStatLoss, item, user, move_user, battle)
  end
  ...
  def self.triggerPriorityChange(item, battler, move, priority)
    trigger(PriorityChange, item, battler, move, priority, ret: priority)
  end

  def self.triggerPriorityBracketChange(item, battler, battle)
    trigger(PriorityBracketChange, item, battler, battle, ret: 0)
  end
  ...
  def self.triggerCriticalCalcFromUser(item, user, target, crit_stage, move = nil)
    trigger(CriticalCalcFromUser, item, user, target, crit_stage, move, ret: crit_stage)
  end

  def self.triggerCriticalCalcFromTarget(item, user, target, crit_stage, move = nil)
    trigger(CriticalCalcFromTarget, item, user, target, crit_stage, move, ret: crit_stage)
  end
  ...
  def self.triggerOnBeingHitPositiveBerry(item, battler, battle, forced)
    trigger(OnBeingHitPositiveBerry, item, battler, battle, forced)
  end
  ...
  def self.triggerOnEndOfUsingMove(item, battler, battle, forced)
    trigger(OnEndOfUsingMove, item, battler, battle, forced)
  end

  def self.triggerOnEndOfUsingMoveStatRestore(item, battler, battle, forced)
    trigger(OnEndOfUsingMoveStatRestore, item, battler, battle, forced)
  end

  #-----------------------------------------------------------------------------

  def self.triggerExpGainModifier(item, battler, exp)
    trigger(ExpGainModifier, item, battler, exp, ret: -1)
  end

  def self.triggerEVGainModifier(item, battler, ev_array)
    return false unless EVGainModifier[item]
  ...
  def self.triggerWeatherExtender(item, weather, duration, battler, battle)
    trigger(WeatherExtender, item, weather, duration, battler, battle, ret: duration)
  end

  def self.triggerTerrainExtender(item, terrain, duration, battler, battle)
    trigger(TerrainExtender, item, terrain, duration, battler, battle, ret: duration)
  end

  def self.triggerOnWeatherChange(item, battler, battle, old_weather)
    trigger(OnWeatherChange, item, battler, battle, old_weather)
  end

  def self.triggerOnTerrainChange(item, battler, battle, old_terrain)
    trigger(OnTerrainChange, item, battler, battle, old_terrain)
  end
  ...
  def self.triggerCertainSwitching(item, switcher, battle)
    trigger(CertainSwitching, item, switcher, battle)
  end

  def self.triggerTrappingByTarget(item, switcher, bearer, battle)
    trigger(TrappingByTarget, item, switcher, bearer, battle)
  end
  ...
  def self.triggerOnIntimidated(item, battler, battle)
    trigger(OnIntimidated, item, battler, battle)
  end

  #-----------------------------------------------------------------------------

  def self.triggerCertainEscapeFromBattle(item, battler)
    trigger(CertainEscapeFromBattle, item, battler)
  end
end
...
Battle::ItemEffects::SpeedCalc.add(:QUICKPOWDER,
                                   proc { |item, battler, mult|
                                     if battler.isSpecies?(:DITTO) && !battler.effects[PBEffects::Transform]
                                       next mult * 2
                                     end
                                   })
...
Battle::ItemEffects::HPHeal.add(:LANSATBERRY,
                                proc { |item, battler, battle, forced|
                                  ...
                                  itemName = GameData::Item.get(item).name
                                  if forced
                                    battle.pbDisplay(_INTL('¡{1} se ha entusiasmado con {2}!', battler.pbThis,
                                                           itemName))
                                  else
                                    battle.pbDisplay(_INTL('¡{1} ha usado {2} y se ha entusiasmado!', battler.pbThis,
                                                           itemName))
                                  end
                                  next true
                                })
...
Battle::ItemEffects::HPHeal.add(:MICLEBERRY,
                                proc { |item, battler, battle, forced|
                                  ...
                                  if forced
                                    ...
                                    battle.pbDisplay(_INTL('¡{1} ha aumentado la precisión de su próximo ataque!',
                                                           battler.pbThis))
                                  else
...
Battle::ItemEffects::HPHeal.add(:ORANBERRY,
                                proc { |item, battler, battle, forced|
                                  ...
                                  if forced
                                    ...
                                    battle.pbDisplay(_INTL('¡{1} ha recuperado unos pocos PS con {2}!', battler.pbThis,
                                                           itemName))
                                  end
                                  next true
                                })
...
Battle::ItemEffects::HPHeal.add(:SITRUSBERRY,
                                proc { |item, battler, battle, forced|
                                  ...
                                  if forced
                                    ...
                                    battle.pbDisplay(_INTL('¡{1} ha restaurado su salud con {2}!', battler.pbThis,
                                                           itemName))
                                  end
                                  next true
                                })
...
Battle::ItemEffects::StatusCure.add(:ASPEARBERRY,
                                    proc { |item, battler, battle, forced|
                                      ...
                                      next false unless %i[FROZEN FROSTBITE].include?(battler.status)
                                      ...
                                      unless forced
                                        battle.pbDisplay(_INTL('¡{1} se ha descongelado con {2}!', battler.pbThis,
                                                               itemName))
                                      end
                                      next true
                                    })
...
Battle::ItemEffects::StatusCure.add(:CHERIBERRY,
                                    proc { |item, battler, battle, forced|
                                      ...
                                      battler.pbCureStatus(forced)
                                      unless forced
                                        battle.pbDisplay(_INTL('¡{1} se ha curado de la parálisis con {2}!',
                                                               battler.pbThis, itemName))
                                      end
                                      next true
                                    })
...
Battle::ItemEffects::StatusCure.add(:CHESTOBERRY,
                                    proc { |item, battler, battle, forced|
                                      ...
                                      battler.pbCureStatus(forced)
                                      unless forced
                                        battle.pbDisplay(_INTL('¡{1} se ha despertado con {2}!', battler.pbThis,
                                                               itemName))
                                      end
                                      next true
                                    })
...
Battle::ItemEffects::StatusCure.add(:LUMBERRY,
                                    proc { |item, battler, battle, forced|
                                      ...
                                      if forced
                                        if oldConfusion
                                          battle.pbDisplay(_INTL('¡{1} ya no está confuso!',
                                                                 battler.pbThis))
                                        end
                                      else
                                        case oldStatus
                                        when :SLEEP
                                          battle.pbDisplay(_INTL('¡{1} se ha despertado con {2}!', battler.pbThis,
                                                                 itemName))
                                        when :POISON
                                          battle.pbDisplay(_INTL('¡{1} se ha curado del envenenamiento con {2}!',
                                                                 battler.pbThis, itemName))
                                        when :BURN
                                          battle.pbDisplay(_INTL('¡{1} se ha curado de las quemaduras con {2}!',
                                                                 battler.pbThis, itemName))
                                        when :PARALYSIS
                                          battle.pbDisplay(_INTL('¡{1} se ha curado de la parálisis con {2}!',
                                                                 battler.pbThis, itemName))
                                        ...
                                        if oldConfusion
                                          battle.pbDisplay(_INTL('¡{1} ya no está confuso gracias a {2}!',
                                                                 battler.pbThis, itemName))
                                        end
                                      end
                                      next true
...
Battle::ItemEffects::StatusCure.add(:MENTALHERB,
                                    proc { |item, battler, battle, forced|
                                      ...
                                      if battler.effects[PBEffects::Attract] >= 0
                                        if forced
                                          battle.pbDisplay(_INTL('¡{1} se ha liberado del enamoramiento!',
                                                                 battler.pbThis))
                                        else
                                      ...
                                      if battler.effects[PBEffects::Taunt] > 0
                                        battle.pbDisplay(_INTL('¡{1} ya se ha olvidado de la mofa!',
                                                               battler.pbThis))
                                      end
                                      battler.effects[PBEffects::Taunt] = 0
                                      if battler.effects[PBEffects::Encore] > 0
                                        battle.pbDisplay(_INTL('¡{1} ya no sufre los efectos de Otra Vez!',
                                                               battler.pbThis))
                                      end
                                      battler.effects[PBEffects::Encore]     = 0
                                      battler.effects[PBEffects::EncoreMove] = nil
                                      if battler.effects[PBEffects::Torment]
                                        battle.pbDisplay(_INTL('¡Tormento ya no surte efecto en {1}!',
                                                               battler.pbThis(true)))
                                      end
                                      battler.effects[PBEffects::Torment] = false
                                      if battler.effects[PBEffects::Disable] > 0
                                        battle.pbDisplay(_INTL('¡El movimiento de {1} ya no está anulado!',
                                                               battler.pbThis(true)))
                                      end
                                      battler.effects[PBEffects::Disable] = 0
                                      if battler.effects[PBEffects::HealBlock] > 0
                                        battle.pbDisplay(_INTL('¡Se han pasado los efectos de Anticura en {1}!',
                                                               battler.pbThis(true)))
                                      end
                                      battler.effects[PBEffects::HealBlock] = 0
                                      next true
...
Battle::ItemEffects::StatusCure.add(:PECHABERRY,
                                    proc { |item, battler, battle, forced|
                                      ...
                                      battler.pbCureStatus(forced)
                                      unless forced
                                        battle.pbDisplay(_INTL('¡{1} se ha curado del envenienmiento con {2}!',
                                                               battler.pbThis, itemName))
                                      end
                                      next true
                                    })
                                          ...
                                          proc { |item, battler, stat, battle, showMessages|
                                            if showMessages
                                              battle.pbDisplay(_INTL('¡Los efectos del {2} de {1} evitan que sus estadísticas bajen!',
                                                                     battler.pbThis, GameData::Item.get(item).name))
                                            end
                                            next true
                                          })
...
Battle::ItemEffects::OnStatLoss.add(:EJECTPACK,
                                    proc { |item, battler, move_user, battle|
                                      ...
                                      next false unless battle.pbCanSwitchOut?(battler.index) # Battler can't switch out
                                      # No Pokémon can switch in
                                      next false unless battle.pbCanChooseNonActive?(battler.index)

                                      battle.pbCommonAnimation('UseItem', battler)
                                      battle.pbDisplay(_INTL('¡{1} se retira por {2}!', battler.pbThis,
                                                             battler.itemName))
...
Battle::ItemEffects::CopyStatChanges.add(:MIRRORHERB,
                                         proc { |item, battler, battle|
                                           ...
                                           battler.allOpposing.each do |b|
                                             b.stagesChangeRecord[0].each_pair do |stat, increment|
                                               raises[stat] += increment
                                             end
                                           end
...
Battle::ItemEffects::PriorityBracketUse.add(:CUSTAPBERRY,
                                            proc { |item, battler, battle|
                                              ...
                                              battle.pbDisplay(_INTL('¡{1} se mueve primero gracias a {2}!',
                                                                     battler.pbThis, battler.itemName))
...
Battle::ItemEffects::OnMissingTarget.add(:BLUNDERPOLICY,
                                         proc { |item, user, target, move, hit_num, battle|
                                           ...
                                           next if %w[OHKO OHKOIce
                                                      OHKOHitsUndergroundTarget].include?(move.function_code)
                                           next unless user.pbCanRaiseStatStage?(:SPEED, user)

                                           battle.pbCommonAnimation('UseItem', user)
                                           user.pbRaiseStatStageByCause(:SPEED, 2, user, user.itemName)
                                           battle.pbDisplay(_INTL('{1} ha desaparecido al haber cumplido su función.',
                                                                  user.itemName))
...
Battle::ItemEffects::DamageCalcFromUser.add(:SOULDEW,
                                            proc { |item, user, target, move, mults, power, type|
                                              ...
                                              if Settings::SOUL_DEW_POWERS_UP_TYPES
                                                mults[:final_damage_multiplier] *= 1.2 if %i[DRAGON
                                                                                             PSYCHIC].include?(type)
...
Battle::ItemEffects::OnBeingHit.add(:AIRBALLOON,
                                    proc { |item, user, target, move, battle|
                                      battle.pbDisplay(_INTL('¡{1} de {2} ha explotado!', target.pbThis,
                                                             target.itemName))
...
Battle::ItemEffects::OnBeingHit.add(:LUMINOUSMOSS,
                                    proc { |item, user, target, move, battle|
                                      next if move.calcType != :WATER
                                      next unless target.pbCanRaiseStatStage?(:SPECIAL_DEFENSE, target)
...
Battle::ItemEffects::OnBeingHit.add(:ROCKYHELMET,
                                    proc { |item, user, target, move, battle|
                                      ...
                                      battle.pbDisplay(_INTL('¡{2} ha dañado a {1}!', user.pbThis(true),
                                                             target.itemName))
                                    })
...
Battle::ItemEffects::OnBeingHitPositiveBerry.add(:ENIGMABERRY,
                                                 proc { |item, battler, battle, forced|
                                                   ...
                                                   if forced
                                                     ...
                                                     battle.pbDisplay(_INTL('¡{1} ha restaurado su salud con {2}!',
                                                                            battler.pbThis, itemName))
                                                   end
                                                   next true
                                                 })
...
Battle::ItemEffects::OnBeingHitPositiveBerry.add(:KEEBERRY,
                                                 proc { |item, battler, battle, forced|
                                                   ...
                                                   battle.pbHideAbilitySplash(battler) if ripening
                                                   unless forced
                                                     next battler.pbRaiseStatStageByCause(:DEFENSE, amt, battler,
                                                                                          itemName)
                                                   end
...
Battle::ItemEffects::OnBeingHitPositiveBerry.add(:MARANGABERRY,
                                                 proc { |item, battler, battle, forced|
                                                   next false if !forced && !battler.canConsumeBerry?
                                                   next false unless battler.pbCanRaiseStatStage?(:SPECIAL_DEFENSE,
                                                                                                  battler)
                                                   ...
                                                   battle.pbHideAbilitySplash(battler) if ripening
                                                   unless forced
                                                     next battler.pbRaiseStatStageByCause(:SPECIAL_DEFENSE, amt, battler,
                                                                                          itemName)
                                                   end
...
Battle::ItemEffects::AfterMoveUseFromTarget.add(:EJECTBUTTON,
                                                proc { |item, battler, user, move, switched_battlers, battle|
                                                  next unless switched_battlers.empty?
                                                  ...
                                                  battle.pbCommonAnimation('UseItem', battler)
                                                  battle.pbDisplay(_INTL('¡{1} regresa gracias a {2}!', battler.pbThis,
                                                                         battler.itemName))
                                                  battler.pbConsumeItem(true, false)
                                                  newPkmn = battle.pbGetReplacementPokemonIndex(battler.index) # Owner chooses
                                                  next if newPkmn < 0
...
Battle::ItemEffects::AfterMoveUseFromTarget.add(:REDCARD,
                                                proc { |item, battler, user, move, switched_battlers, battle|
                                                  ...
                                                  battler.pbConsumeItem
                                                  next unless user.canBeForcedOutOfBattle?

                                                  battle.pbRecallAndReplace(user.index, newPkmn, true)
                                                  battle.pbDisplay(_INTL('¡{1} fue arrastrado al combate!',
                                                                         user.pbThis))
...
Battle::ItemEffects::OnEndOfUsingMove.add(:LEPPABERRY,
                                          proc { |item, battler, battle, forced|
                                            ...
                                            if found_empty_moves.empty? && (!forced || found_partial_moves.empty?)
                                              next false
                                            end
                                            ...
                                            if forced
                                              battle.pbDisplay(_INTL('¡{1} ha recuperado los PP de {2}!',
                                                                     battler.pbThis, moveName))
                                            else
                                              battle.pbDisplay(_INTL('¡{1} ha recuperado los PP de {3} gracias a {2}!',
                                                                     battler.pbThis, itemName, moveName))
                                            end
                                            next true
                                          })
                                                     ...
                                                     proc { |item, battler, battle, forced|
                                                       ...
                                                       if forced
                                                         battle.pbDisplay(_INTL(
                                                                            '¡Se ha restaurado las características de {1}!', battler.pbThis(true)
                                                                          ))
                                                       else
...
Battle::ItemEffects::WeatherExtender.add(:ICYROCK,
                                         proc { |item, weather, duration, battler, battle|
                                           next 8 if %i[Hail Snowstorm].include?(weather)
                                         })
...
Battle::ItemEffects::OnWeatherChange.add(:BOOSTERENERGY,
                                         proc { |item, battler, battle, old_weather|
                                           ...
                                           next false unless battler.hasActiveAbility?(:PROTOSYNTHESIS)
                                           next false unless %i[Sun HarshSun].include?(battle.field.pbWeather)
...
Battle::ItemEffects::OnTerrainChange.add(:BOOSTERENERGY,
                                         proc { |item, battler, battle, old_terrain|
                                           ...
                                           next false if battler.effects[PBEffects::ProtosynthesisStat]
                                           next false if battler.effects[PBEffects::BoosterEnergy]
                                           next false unless battler.hasActiveAbility?(:QUARKDRIVE)
                                           ...
                                           proc { |item, battler, battle|
                                             if battler.pbHasType?(:POISON)
                                               ...
                                               battler.pbTakeEffectDamage(battler.totalhp / 8) do |hp_lost|
                                                 battle.pbDisplay(_INTL('¡{1} ha resultado dañado por {2}!',
                                                                        battler.pbThis, battler.itemName))
                                               end
                                             end
                                           })
...
Battle::ItemEffects::EndOfRoundEffect.add(:FLAMEORB,
                                          proc { |item, battler, battle|
                                            next unless battler.pbCanBurn?(battler, false)
...
Battle::ItemEffects::EndOfRoundEffect.add(:TOXICORB,
                                          proc { |item, battler, battle|
                                            next unless battler.pbCanPoison?(battler, false)
...
Battle::ItemEffects::OnSwitchIn.add(:BOOSTERENERGY,
                                    proc { |item, battler, battle|
                                      ...
                                      Battle::ItemEffects.triggerOnWeatherChange(item, battler, battle,
                                                                                 battle.field.weather)
                                      Battle::ItemEffects.triggerOnTerrainChange(item, battler, battle,
...
Battle::ItemEffects::OnIntimidated.add(:ADRENALINEORB,
                                       proc { |item, battler, battle|
                                         next false unless battler.pbCanRaiseStatStage?(:SPEED, battler)
