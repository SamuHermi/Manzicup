...
module Battle::AbilityEffects
  ...
  def self.triggerStatusCure(ability, battler)
    trigger(StatusCure, ability, battler)
  end

  #=============================================================================

  def self.triggerStatLossImmunity(ability, battler, stat, battle, show_messages)
    trigger(StatLossImmunity, ability, battler, stat, battle, show_messages)
  end

  def self.triggerStatLossImmunityNonIgnorable(ability, battler, stat, battle, show_messages)
    trigger(StatLossImmunityNonIgnorable, ability, battler, stat, battle, show_messages)
  end

  def self.triggerStatLossImmunityFromAlly(ability, bearer, battler, stat, battle, show_messages)
    trigger(StatLossImmunityFromAlly, ability, bearer, battler, stat, battle, show_messages)
  end
  ...
  def self.triggerPriorityChange(ability, battler, move, priority)
    trigger(PriorityChange, ability, battler, move, priority, ret: priority)
  end

  def self.triggerPriorityBracketChange(ability, battler, battle)
    trigger(PriorityBracketChange, ability, battler, battle, ret: 0)
  end
  ...
  def self.triggerMoveBlocking(ability, bearer, user, targets, move, battle)
    trigger(MoveBlocking, ability, bearer, user, targets, move, battle)
  end

  def self.triggerMoveImmunity(ability, user, target, move, type, battle, show_message)
    trigger(MoveImmunity, ability, user, target, move, type, battle, show_message)
  end

  #=============================================================================

  def self.triggerModifyMoveBaseType(ability, user, move, type)
    trigger(ModifyMoveBaseType, ability, user, move, type, ret: type)
  end
  ...
  def self.triggerCriticalCalcFromUser(ability, user, target, crit_stage)
    trigger(CriticalCalcFromUser, ability, user, target, crit_stage, ret: crit_stage)
  end

  def self.triggerCriticalCalcFromTarget(ability, user, target, crit_stage)
    trigger(CriticalCalcFromTarget, ability, user, target, crit_stage, ret: crit_stage)
  end
  ...
  def self.triggerCertainSwitching(ability, switcher, battle)
    trigger(CertainSwitching, ability, switcher, battle)
  end

  def self.triggerTrappingByTarget(ability, switcher, bearer, battle)
    trigger(TrappingByTarget, ability, switcher, bearer, battle)
  end
  ...
  def self.triggerOnSwitchIn(ability, battler, battle, switch_in = false)
    ...
    battle.allSameSideBattlers(battler.index).each do |b|
      next unless b.hasActiveAbility?(:COMMANDER)
  ...
  def self.triggerModifyTypeEffectiveness(ability, user, target, move, battle, effectiveness)
    trigger(ModifyTypeEffectiveness, ability, user, target, move, battle, effectiveness, ret: effectiveness)
  end
  ...
  def self.triggerCertainEscapeFromBattle(ability, battler)
    trigger(CertainEscapeFromBattle, ability, battler)
  end
end
...
Battle::AbilityEffects::SpeedCalc.add(:CHLOROPHYLL,
                                      proc { |ability, battler, mult|
                                        next mult * 2 if %i[Sun HarshSun].include?(battler.effectiveWeather)
                                      })
...
Battle::AbilityEffects::SpeedCalc.add(:SWIFTSWIM,
                                      proc { |ability, battler, mult|
                                        next mult * 2 if %i[Rain HeavyRain].include?(battler.effectiveWeather)
                                      })
...
Battle::AbilityEffects::OnHPDroppedBelowHalf.add(:EMERGENCYEXIT,
                                                 proc { |ability, battler, move_user, battle|
                                                   ...
                                                   if battle.wildBattle?
                                                     if battler.opposes? && battle.pbSideBattlerCount(battler.index) > 1
                                                       next false
                                                     end
                                                   ...
                                                   # Battler can't switch out
                                                   next false unless battle.pbCanSwitchOut?(battler.index)
                                                   # No Pokémon can switch in
                                                   next false unless battle.pbCanChooseNonActive?(battler.index)

                                                   battle.pbShowAbilitySplash(battler, true)
                                                   battle.pbHideAbilitySplash(battler)
                                                   unless Battle::Scene::USE_ABILITY_SPLASH
                                                     battle.pbDisplay(_INTL('¡La habilidad {2} de {1} se ha activado!',
                                                   ...
                                                   if battle.endOfRound # Just switch out
                                                     battle.scene.pbRecall(battler.index) unless battler.fainted?
                                                   ...
                                                   if move_user && battler.index == move_user.index
...
Battle::AbilityEffects::StatusCheckNonIgnorable.add(:COMATOSE,
                                                    proc { |ability, battler, status|
                                                      next false unless battler.isSpecies?(:KOMALA)
...
Battle::AbilityEffects::StatusImmunity.add(:LEAFGUARD,
                                           proc { |ability, battler, status|
                                             next true if %i[Sun HarshSun].include?(battler.effectiveWeather)
                                           })
                                              ...
                                              proc { |ability, battler, user, status|
                                                ...
                                                when :POISON
                                                  if user.pbCanPoisonSynchronize?(battler)
                                                    ...
                                                    msg = nil
                                                    unless Battle::Scene::USE_ABILITY_SPLASH
                                                      msg = _INTL('¡La habilidad {2} de {1} envenenó a {3}!', battler.pbThis, battler.abilityName,
                                                                  user.pbThis(true))
                                                    end
                                                ...
                                                when :BURN
                                                  if user.pbCanBurnSynchronize?(battler)
                                                    ...
                                                    msg = nil
                                                    unless Battle::Scene::USE_ABILITY_SPLASH
                                                      msg = _INTL('¡La habilidad {2} de {1} quemó a {3}!',
                                                ...
                                                when :FROSTBITE
                                                  if user.pbCanSynchronizeStatus?(:FROZEN, battler)
                                                    ...
                                                    msg = nil
                                                    unless Battle::Scene::USE_ABILITY_SPLASH
                                                      msg = _INTL('¡{2} de {1} heló a {3}!', battler.pbThis,
                                                                  battler.abilityName, user.pbThis(true))
                                                    end
...
Battle::AbilityEffects::StatusCure.add(:IMMUNITY,
                                       proc { |ability, battler|
                                         ...
                                         unless Battle::Scene::USE_ABILITY_SPLASH
                                           battler.battle.pbDisplay(_INTL('¡La habilidad {2} de {1} curó el envenenamiento!', battler.pbThis,
                                                                          battler.abilityName))
                                         end
                                         battler.battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::StatusCure.add(:INSOMNIA,
                                       proc { |ability, battler|
                                         ...
                                         battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
                                         unless Battle::Scene::USE_ABILITY_SPLASH
                                           battler.battle.pbDisplay(_INTL('¡La habilidad {2} de {1} lo despertó!',
                                                                          battler.pbThis, battler.abilityName))
                                         end
                                         battler.battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::StatusCure.add(:LIMBER,
                                       proc { |ability, battler|
                                         ...
                                         battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
                                         unless Battle::Scene::USE_ABILITY_SPLASH
                                           battler.battle.pbDisplay(_INTL('¡La habilidad {2} de {1} curó su parálisis!', battler.pbThis,
                                                                          battler.abilityName))
                                         end
                                         battler.battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::StatusCure.add(:MAGMAARMOR,
                                       proc { |ability, battler|
                                         next unless %i[FROZEN FROSTBITE].include?(battler.status)

                                         battler.battle.pbShowAbilitySplash(battler)
                                         battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
                                         unless Battle::Scene::USE_ABILITY_SPLASH
                                           battler.battle.pbDisplay(_INTL('¡La habilidad {2} de {1} lo descongeló!',
                                                                          battler.pbThis, battler.abilityName))
                                         end
                                         battler.battle.pbHideAbilitySplash(battler)
                                       })
                                       ...
                                       proc { |ability, battler|
                                         ...
                                         if battler.effects[PBEffects::Attract] >= 0
                                           ...
                                           if Battle::Scene::USE_ABILITY_SPLASH
                                             battler.battle.pbDisplay(_INTL('{1} dejó de estar enamorado.',
                                                                            battler.pbThis))
                                           else
                                         ...
                                         if battler.effects[PBEffects::Taunt] > 0 && Settings::MECHANICS_GENERATION >= 6
                                           ...
                                           if Battle::Scene::USE_ABILITY_SPLASH
                                             battler.battle.pbDisplay(_INTL(
                                                                        '¡El efecto de Mofa sobre {1} desapareció!', battler.pbThis
                                                                      ))
                                           else
...
Battle::AbilityEffects::StatusCure.add(:WATERVEIL,
                                       proc { |ability, battler|
                                         ...
                                         battler.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
                                         unless Battle::Scene::USE_ABILITY_SPLASH
                                           battler.battle.pbDisplay(_INTL('¡La habilidad {2} de {1} curó la quemadura!', battler.pbThis,
                                                                          battler.abilityName))
                                         end
                                         battler.battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::StatLossImmunity.add(:BIGPECKS,
                                             proc { |ability, battler, stat, battle, showMessages|
                                               ...
                                               if showMessages
                                                 ...
                                                 if Battle::Scene::USE_ABILITY_SPLASH
                                                   battle.pbDisplay(_INTL('¡{2} de {1} no puede ser bajado!',
                                                                          battler.pbThis(true), GameData::Stat.get(stat).name))
                                                 else
                                               ...
                                               if showMessages
                                                 ...
                                                 if Battle::Scene::USE_ABILITY_SPLASH
                                                   battle.pbDisplay(_INTL(
                                                                      '¡Las estadísticas de {1} no pueden ser bajadas!', battler.pbThis
                                                                    ))
                                                 else
                                                   battle.pbDisplay(_INTL('¡La habilidad {2} de {1} evita la bajada de estadísticas!', battler.pbThis,
                                                                          battler.abilityName))
                                                 end
                                                 battle.pbHideAbilitySplash(battler)
                                               end
...
Battle::AbilityEffects::PriorityBracketChange.add(:MYCELIUMMIGHT,
                                                  proc { |ability, battler, battle|
                                                    ...
                                                    next -1 if (choices[0] == :UseMove) && choices[2].statusMove?
                                                  })
...
Battle::AbilityEffects::PriorityBracketUse.add(:QUICKDRAW,
                                               proc { |ability, battler, battle|
                                                 battle.pbShowAbilitySplash(battler)
                                                 battle.pbDisplay(_INTL('¡La habilidad {2} de {1} hizó que se mueva más rápido!', battler.abilityName,
                                                                        battler.pbThis(true)))
                                                 battle.pbHideAbilitySplash(battler)
                                               })
...
Battle::AbilityEffects::MoveImmunity.add(:COMMANDER,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           next false unless target.isCommander?

                                           if show_message
                                             battle.pbDisplay(_INTL('¡{1} esquivó el ataque!',
                                                                    target.pbThis))
                                           end
                                           next true
                                         })
...
Battle::AbilityEffects::MoveImmunity.add(:EARTHEATER,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           next target.pbMoveImmunityHealingAbility(user, move, type, :GROUND,
                                                                                    show_message)
                                         })
                                         ...
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           ...
                                           if show_message
                                             ...
                                             if !target.effects[PBEffects::FlashFire]
                                               ...
                                               if Battle::Scene::USE_ABILITY_SPLASH
                                                 battle.pbDisplay(_INTL(
                                                                    '¡El poder de los ataques de tipo Fuego de {1} aumentó!', target.pbThis(true)
                                                                  ))
                                               else
...
Battle::AbilityEffects::MoveImmunity.add(:GOODASGOLD,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           next false unless move.statusMove?
                                           next false if user.index == target.index
...
Battle::AbilityEffects::MoveImmunity.add(:SOUNDPROOF,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           ...
                                           if Settings::MECHANICS_GENERATION >= 8 && user.index == target.index
                                             next false
                                           end
                                           ...
                                           if show_message
                                             ...
                                             if Battle::Scene::USE_ABILITY_SPLASH
                                               ...
                                               battle.pbDisplay(_INTL('¡La habilidad {2} de {1} bloqueó {3}!',
                                                                      target.pbThis, target.abilityName, move.name))
                                             end
                                             battle.pbHideAbilitySplash(target)
                                           end
...
Battle::AbilityEffects::MoveImmunity.add(:TELEPATHY,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           ...
                                           if show_message
                                             battle.pbShowAbilitySplash(target)
                                             if Battle::Scene::USE_ABILITY_SPLASH
                                               battle.pbDisplay(_INTL('¡{1} esquiva ataques de su Pokémon aliado!',
                                                                      target.pbThis(true)))
                                             else
...
Battle::AbilityEffects::MoveImmunity.add(:VOLTABSORB,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           next target.pbMoveImmunityHealingAbility(user, move, type, :ELECTRIC,
                                                                                    show_message)
                                         })

Battle::AbilityEffects::MoveImmunity.add(:WATERABSORB,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           next target.pbMoveImmunityHealingAbility(user, move, type, :WATER,
                                                                                    show_message)
                                         })
...
Battle::AbilityEffects::MoveImmunity.add(:WELLBAKEDBODY,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           next target.pbMoveImmunityStatRaisingAbility(user, move, type, :FIRE,
                                                                                        :DEFENSE, 2, show_message)
                                         })
                                         ...
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           ...
                                           if show_message
                                             ...
                                             if target.pbCanRaiseStatStage?(:ATTACK, user, move)
                                               ...
                                               battle.pbDisplay(_INTL('¡{1} de {2} hizo inefectivo el {3}!',
                                                                      target.abilityName, target.pbThis, move.name))
                                             end
                                             battle.pbHideAbilitySplash(target)
                                           end
                                           next true
...
Battle::AbilityEffects::MoveImmunity.add(:WONDERGUARD,
                                         proc { |ability, user, target, move, type, battle, show_message|
                                           next false if move.statusMove?
                                           if !type || Effectiveness.super_effective?(target.damageState.typeMod)
                                             next false
                                           end
                                           ...
                                           if show_message
                                             ...
                                             if Battle::Scene::USE_ABILITY_SPLASH
                                               ...
                                               battle.pbDisplay(_INTL('¡{1} evitó daño con la habilidad {2}!',
                                                                      target.pbThis, target.abilityName))
                                             end
                                             battle.pbHideAbilitySplash(target)
                                           end
...
Battle::AbilityEffects::AccuracyCalcFromUser.add(:KEENEYE,
                                                 proc { |ability, mods, user, target, move, type|
                                                   if mods[:evasion_stage] > 0 && Settings::MECHANICS_GENERATION >= 6
                                                     mods[:evasion_stage] =
                                                       0
                                                   end
                                                 })
...
Battle::AbilityEffects::DamageCalcFromUser.add(:FLOWERGIFT,
                                               proc { |ability, user, target, move, mults, power, type|
                                                 if move.physicalMove? && %i[Sun
                                                                             HarshSun].include?(user.effectiveWeather)
                                                   mults[:attack_multiplier] *= 1.5
                                                 end
                                               })
...
Battle::AbilityEffects::DamageCalcFromUser.add(:MINUS,
                                               proc { |ability, user, target, move, mults, power, type|
                                                 ...
                                                 if user.allAllies.any? { |b| b.hasActiveAbility?(%i[MINUS PLUS]) }
                                                   mults[:attack_multiplier] *= 1.5
                                                 end
                                               })
...
Battle::AbilityEffects::DamageCalcFromUser.add(:ORICHALCUMPULSE,
                                               proc { |ability, user, target, move, mults, baseDmg, type|
                                                 mults[:attack_multiplier] *= 4 / 3.0 if move.physicalMove? && %i[Sun
                                                                                                                  HarshSun].include?(user.effectiveWeather)
                                               })
...
Battle::AbilityEffects::DamageCalcFromUser.add(:PROTOSYNTHESIS,
                                               proc { |ability, user, target, move, mults, baseDmg, type|
                                                 ...
                                                 if move.physicalMove? && stat == :ATTACK
                                                   mults[:attack_multiplier] *= 1.3
                                                 end
                                                 if move.specialMove? && stat == :SPECIAL_ATTACK
                                                   mults[:attack_multiplier] *= 1.3
                                                 end
                                               })
...
Battle::AbilityEffects::DamageCalcFromUser.add(:RIVALRY,
                                               proc { |ability, user, target, move, mults, power, type|
                                                 if user.gender != 2 && target.gender != 2
                                                   mults[:power_multiplier] *= if user.gender == target.gender
                                                                                 1.25
                                                                               else
                                                                                 0.75
                                                                               end
...
Battle::AbilityEffects::DamageCalcFromUser.add(:SLOWSTART,
                                               proc { |ability, user, target, move, mults, power, type|
                                                 if user.effects[PBEffects::SlowStart] > 0
                                                   mults[:attack_multiplier] /= 2
                                                 end # && move.physicalMove?
                                               })
...
Battle::AbilityEffects::DamageCalcFromUser.add(:TOXICBOOST,
                                               proc { |ability, user, target, move, mults, power, type|
                                                 mults[:power_multiplier] *= 1.5 if user.poisoned? && move.physicalMove?
                                               })
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:FLOWERGIFT,
                                                 proc { |ability, user, target, move, mults, power, type|
                                                   if move.specialMove? && %i[Sun
                                                                              HarshSun].include?(target.effectiveWeather)
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:PROTOSYNTHESIS,
                                                 proc { |ability, user, target, move, mults, baseDmg, type|
                                                   ...
                                                   if move.physicalMove? && stat == :DEFENSE
                                                     mults[:defense_multiplier] *= 1.3
                                                   end
                                                   if move.specialMove? && stat == :SPECIAL_DEFENSE
                                                     mults[:defense_multiplier] *= 1.3
                                                   end
                                                 })
...
Battle::AbilityEffects::DamageCalcFromTarget.add(:THICKFAT,
                                                 proc { |ability, user, target, move, mults, power, type|
                                                   mults[:power_multiplier] /= 2 if %i[FIRE ICE].include?(type)
                                                 })
                                       ...
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         unless battle.moldBreaker
                                           ...
                                           if dampBattler
                                             ...
                                             if Battle::Scene::USE_ABILITY_SPLASH
                                               battle.pbDisplay(_INTL('¡{1} no puede usar la habilidad {2}!',
                                                                      target.pbThis, target.abilityName))
                                             else
                                         ...
                                         if user.takesIndirectDamage?(Battle::Scene::USE_ABILITY_SPLASH) &&
                                           ...
                                           battle.pbDisplay(_INTL('¡{1} fue atrapado en las consecuencias!',
                                                                  user.pbThis))
                                         end
                                         battle.pbHideAbilitySplash(target)
                                       })
...
Battle::AbilityEffects::OnBeingHit.add(:COTTONDOWN,
                                       proc { |ability, user, target, move, battle|
                                         next if battle.allBattlers.none? do |b|
                                           b.index != target.index && b.pbCanLowerStatStage?(:SPEED, target)
                                         end

                                         battle.pbShowAbilitySplash(target)
                                         battle.allBattlers.each do |b|
                                           if b.index != target.index
                                             b.pbLowerStatStageByAbility(:SPEED, 1, target,
                                                                         false)
                                           end
                                         end
                                         battle.pbHideAbilitySplash(target)
...
Battle::AbilityEffects::OnBeingHit.add(:CURSEDBODY,
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         battle.pbShowAbilitySplash(target)
                                         unless move.pbMoveFailedAromaVeil?(target, user, Battle::Scene::USE_ABILITY_SPLASH)
                                           user.effects[PBEffects::Disable]     = 3
                                           user.effects[PBEffects::DisableMove] = regularMove.id
                                           if Battle::Scene::USE_ABILITY_SPLASH
                                             battle.pbDisplay(_INTL('¡{1} de {2} fue deshabilitado!', user.pbThis,
                                                                    regularMove.name))
                                           else
...
Battle::AbilityEffects::OnBeingHit.add(:ELECTROMORPHOSIS,
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         battle.pbDisplay(_INTL('¡Ser golpeado por {1} cargó a {2} de poder!',
                                                                move.name, target.pbThis(true)))
                                         battle.pbHideAbilitySplash(target)
                                       })
...
Battle::AbilityEffects::OnBeingHit.add(:FLAMEBODY,
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         if user.pbCanBurn?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
                                           ...
                                           msg = nil
                                           unless Battle::Scene::USE_ABILITY_SPLASH
                                             msg = _INTL('¡{1} de {2} quemó a {3}!', target.pbThis, target.abilityName,
                                                         user.pbThis(true))
                                           end
...
Battle::AbilityEffects::OnBeingHit.add(:ILLUSION,
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         next unless target.effects[PBEffects::Illusion]
...
Battle::AbilityEffects::OnBeingHit.add(:MUMMY,
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         next if %i[MUMMY LINGERINGAROMA].include?(user.ability)
                                         ...
                                         if user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
                                           ...
                                           if Battle::Scene::USE_ABILITY_SPLASH
                                             case ability
                                             when :MUMMY
                                               msg = _INTL('¡La habilidad de {1} se convirtió en {2}!', user.pbThis,
                                                           user.abilityName)
                                             when :LINGERINGAROMA
...
Battle::AbilityEffects::OnBeingHit.add(:PERISHBODY,
                                       proc { |ability, user, target, move, battle|
                                         next unless move.pbContactMove?(user)
                                         next if user.fainted?
                                         if user.effects[PBEffects::PerishSong] > 0 || target.effects[PBEffects::PerishSong] > 0
                                           next
                                         end
...
Battle::AbilityEffects::OnBeingHit.add(:POISONPOINT,
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         if user.pbCanPoison?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
                                           ...
                                           msg = nil
                                           unless Battle::Scene::USE_ABILITY_SPLASH
                                             msg = _INTL('¡La habilidad {2} de {1} envenenó a {3}!', target.pbThis,
                                                         target.abilityName, user.pbThis(true))
                                           end
                                           user.pbPoison(target, msg)
                                         end
...
Battle::AbilityEffects::OnBeingHit.add(:TOXICDEBRIS,
                                       proc { |ability, user, target, move, battle|
                                         ...
                                         battle.pbDisplay(_INTL(
                                                            'Púas tóxicas se esparcieron en el suelo alrededor de {1}!', target.pbOpposingTeam(true)
                                                          ))
                                         battle.pbHideAbilitySplash(target)
                                       })
...
Battle::AbilityEffects::OnBeingHit.add(:WANDERINGSPIRIT,
                                       proc { |ability, user, target, move, battle|
                                         next unless move.pbContactMove?(user)
                                         next if user.ungainableAbility? || %i[RECEIVER
                                                                               WONDERGUARD].include?(user.ability_id)
                                         next if user.uncopyableAbility?
                                         if user.hasActiveItem?(:ABILITYSHIELD) || target.hasActiveItem?(:ABILITYSHIELD)
                                           next
                                         end
                                         ...
                                         if user.affectedByContactEffect?(Battle::Scene::USE_ABILITY_SPLASH)
                                           ...
                                           if Battle::Scene::USE_ABILITY_SPLASH
                                             battle.pbDisplay(_INTL('¡{1} ha cambiado habilidades con {2}!',
...
Battle::AbilityEffects::OnBeingHit.add(:SPICYSPRAY,
                                       proc { |ability, user, target, move, battle|
                                         next if !move.specialMove?(user) &&
                                                 !move.physicalMove?(user)
                                         next if user.burned? || battle.pbRandom(100) >= 100
                                         ...
                                         if user.pbCanBurn?(target, Battle::Scene::USE_ABILITY_SPLASH) &&
                                           ...
                                           msg = nil
                                           unless Battle::Scene::USE_ABILITY_SPLASH
                                             msg = _INTL('{2} de {1} quemó a {3}!', target.pbThis, target.abilityName,
                                                         user.pbThis(true))
                                           end
                                           user.pbBurn(target, msg)
                                         end
...
Battle::AbilityEffects::OnDealingHit.add(:POISONTOUCH,
                                         proc { |ability, user, target, move, battle|
                                           ...
                                           if target.hasActiveAbility?(:SHIELDDUST) && !battle.moldBreaker
                                             ...
                                             msg = nil
                                             unless Battle::Scene::USE_ABILITY_SPLASH
                                               msg = _INTL('¡La habilidad {2} de {1} envenenó a {3}!', user.pbThis,
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:BATTLEBOND,
                                             proc { |ability, user, targets, move, battle|
                                               ...
                                               battle.pbDisplay(_INTL(
                                                                  '¡{1} se cargó completamente por el lazo con su entrenador!', user.pbThis
                                                                ))
                                               battle.pbHideAbilitySplash(user)
                                               showAnim = true
                                               %i[ATTACK SPECIAL_ATTACK SPEED].each do |stat|
                                               ...
                                               if showAnim
                                                 battle.pbDisplay(_INTL(
                                                                    '¡Las características de {1} no pueden subir más!', user.pbThis
                                                                  ))
                                               end
                                             })
...
Battle::AbilityEffects::OnEndOfUsingMove.add(:GRIMNEIGH,
                                             proc { |ability, user, targets, move, battle|
                                               ...
                                               numFainted = 0
                                               targets.each { |b| numFainted += 1 if b.damageState.fainted }
                                               next if numFainted == 0 || !user.pbCanRaiseStatStage?(:SPECIAL_ATTACK,
                                                                                                     user)
                                             ...
                                             proc { |ability, user, targets, move, battle|
                                               ...
                                               targets.each do |b|
                                                 ...
                                                 if b.hasActiveAbility?(:STICKYHOLD)
                                                   ...
                                                   if Battle::Scene::USE_ABILITY_SPLASH
                                                     battle.pbDisplay(_INTL('¡El objeto de {1} no puede ser robado!',
                                                                            b.pbThis))
                                                   end
...
Battle::AbilityEffects::AfterMoveUseFromTarget.add(:ANGERSHELL,
                                                   proc { |ability, target, user, move, switched_battlers, battle|
                                                     next unless move.damagingMove?
                                                     next unless target.droppedBelowHalfHP

                                                     showAnim = true
                                                     battle.pbShowAbilitySplash(target)
                                                     %i[ATTACK SPECIAL_ATTACK SPEED].each do |stat|
                                                       next unless target.pbCanRaiseStatStage?(stat, user, nil, true)
                                                     ...
                                                     showAnim = true
                                                     %i[DEFENSE SPECIAL_DEFENSE].each do |stat|
                                                       next unless target.pbCanLowerStatStage?(stat, user, nil, true)
...
Battle::AbilityEffects::AfterMoveUseFromTarget.add(:BERSERK,
                                                   proc { |ability, target, user, move, switched_battlers, battle|
                                                     next unless move.damagingMove?
                                                     next unless target.droppedBelowHalfHP
                                                     next unless target.pbCanRaiseStatStage?(:SPECIAL_ATTACK, target)
...
Battle::AbilityEffects::AfterMoveUseFromTarget.add(:COLORCHANGE,
                                                   proc { |ability, target, user, move, switched_battlers, battle|
                                                     ...
                                                     if !move.calcType || GameData::Type.get(move.calcType).pseudo_type
                                                       next
                                                     end
                                                     if target.pbHasType?(move.calcType) && !target.pbHasOtherType?(move.calcType)
                                                       next
                                                     end
...
Battle::AbilityEffects::AfterMoveUseFromTarget.add(:PICKPOCKET,
                                                   proc { |ability, target, user, move, switched_battlers, battle|
                                                     ...
                                                     next if target.wild?
                                                     # User was switched out
                                                     next if switched_battlers.include?(user.index)
                                                     next unless move.contactMove?
                                                     if user.effects[PBEffects::Substitute] > 0 || target.damageState.substitute
                                                       next
                                                     end
                                                     next if target.item || !user.item
                                                     if user.unlosableItem?(user.item) || target.unlosableItem?(user.item)
                                                       next
                                                     end
                                                     ...
                                                     if user.hasActiveAbility?(:STICKYHOLD)
                                                       ...
                                                       if Battle::Scene::USE_ABILITY_SPLASH
                                                         battle.pbDisplay(_INTL(
                                                                            '¡El objeto de {1} no puede ser robado!', user.pbThis
                                                                          ))
                                                       end
                                                     ...
                                                     if user.hasActiveAbility?(:UNBURDEN)
                                                       user.effects[PBEffects::Unburden] =
                                                         true
                                                     end
                                              ...
                                              proc { |ability, weather, battler, battle|
                                                ...
                                                when :Sun, :HarshSun
                                                  if battler.takesIndirectDamage?
                                                    ...
                                                    battle.pbDisplay(_INTL('¡{1} fue herido por la luz solar!',
                                                                           battler.pbThis))
                                                    battle.pbHideAbilitySplash(battler)
                                                ...
                                                when :Rain, :HeavyRain
                                                  ...
                                                  if Battle::Scene::USE_ABILITY_SPLASH
                                                    battle.pbDisplay(_INTL('Los PS de {1} han sido restaurados.',
                                                                           battler.pbThis))
                                                  else
                                                    battle.pbDisplay(_INTL('La habilidad {2} de {1} restauró sus PS.',
                                                                           battler.pbThis, battler.abilityName))
                                                  end
                                                  battle.pbHideAbilitySplash(battler)
                                                end
...
Battle::AbilityEffects::EndOfRoundWeather.add(:ICEFACE,
                                              proc { |ability, weather, battler, battle|
                                                ...
                                                battle.pbShowAbilitySplash(battler)
                                                unless Battle::Scene::USE_ABILITY_SPLASH
                                                  battle.pbDisplay(_INTL('¡La habilidad {2} de {1} se activó!',
                                                                         battler.pbThis, battler.abilityName))
                                                end
...
Battle::AbilityEffects::EndOfRoundWeather.add(:RAINDISH,
                                              proc { |ability, weather, battler, battle|
                                                next unless %i[Rain HeavyRain].include?(weather)
                                                next unless battler.canHeal?
...
Battle::AbilityEffects::EndOfRoundWeather.add(:SOLARPOWER,
                                              proc { |ability, weather, battler, battle|
                                                next unless %i[Sun HarshSun].include?(weather)
                                              ...
                                              proc { |ability, battler, battle|
                                                ...
                                                battler.allAllies.each do |b|
                                                  ...
                                                  unless Battle::Scene::USE_ABILITY_SPLASH
                                                    case oldStatus
                                                    when :SLEEP
                                                      battle.pbDisplay(_INTL('¡La habilidad {2} de {1} despertó a su compañero!', battler.pbThis,
                                                                             battler.abilityName))
                                                    when :POISON
                                                      battle.pbDisplay(_INTL('¡La habilidad {2} de {1} curó el envenamiento de su compañero!', battler.pbThis,
                                                                             battler.abilityName))
                                                    when :BURN
                                                      battle.pbDisplay(_INTL('¡La habilidad {2} de {1} curó la quemadura de su compañero!', battler.pbThis,
                                                                             battler.abilityName))
                                                    when :PARALYSIS
                                                      battle.pbDisplay(_INTL('¡La habilidad {2} de {1} curó la parálisis de su compañero!', battler.pbThis,
                                                                             battler.abilityName))
                                                    when :FROZEN
                                                      battle.pbDisplay(_INTL('¡La habilidad {2} de {1} descongelo a su compañero!', battler.pbThis,
                                                                             battler.abilityName))
                                                    when :FROSTBITE
                                                      battle.pbDisplay(_INTL('¡La habilidad {2} de {1} descongelo a su compañero!', battler.pbThis,
                                                                             battler.abilityName))
...
Battle::AbilityEffects::EndOfRoundHealing.add(:HYDRATION,
                                              proc { |ability, battler, battle|
                                                ...
                                                next unless %i[Rain HeavyRain].include?(battler.effectiveWeather)
                                                ...
                                                unless Battle::Scene::USE_ABILITY_SPLASH
                                                  case oldStatus
                                                  when :SLEEP
                                                    battle.pbDisplay(_INTL('¡La habilidad {2} de {1} le despertó!',
                                                                           battler.pbThis, battler.abilityName))
                                                  when :POISON
                                                    battle.pbDisplay(_INTL(
                                                                       '¡La habilidad {2} de {1} curó su envenamiento!', battler.pbThis, battler.abilityName
                                                                     ))
                                                  when :BURN
                                                    battle.pbDisplay(_INTL(
                                                                       '¡La habilidad {2} de {1} curó su quemadura!', battler.pbThis, battler.abilityName
                                                                     ))
                                                  when :PARALYSIS
                                                    battle.pbDisplay(_INTL(
                                                                       '¡La habilidad {2} de {1} curó su parálisis!', battler.pbThis, battler.abilityName
                                                                     ))
                                                  when :FROZEN
                                                    battle.pbDisplay(_INTL('¡La habilidad {2} de {1} le descongeló!',
                                                                           battler.pbThis, battler.abilityName))
                                                  when :FROSTBITE
                                                    battle.pbDisplay(_INTL('¡La habilidad {2} de {1} le descongeló!',
                                                                           battler.pbThis, battler.abilityName))
...
Battle::AbilityEffects::EndOfRoundEffect.add(:BADDREAMS,
                                             proc { |ability, battler, battle|
                                               battle.allOtherSideBattlers(battler.index).each do |b|
                                                 ...
                                                 battle.pbShowAbilitySplash(battler)
                                                 next unless b.takesIndirectDamage?(Battle::Scene::USE_ABILITY_SPLASH)

                                                 b.pbTakeEffectDamage(b.totalhp / 8) do |hp_lost|
                                                   if Battle::Scene::USE_ABILITY_SPLASH
                                                     battle.pbDisplay(_INTL('¡{1} está inmerso en un sueño agitado!',
                                                                            b.pbThis))
                                                   else
...
Battle::AbilityEffects::EndOfRoundEffect.add(:CUDCHEW,
                                             proc { |ability, battler, battle|
                                               ...
                                               if !battler.recycleItem || !GameData::Item.get(battler.recycleItem).is_berry?
                                                 next
                                               end
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:BALLFETCH,
                                               proc { |ability, battler, battle|
                                                 ...
                                                 battle.pbDisplay(_INTL('¡{1} ha recuperado la {2} lanzada!',
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:HARVEST,
                                               proc { |ability, battler, battle|
                                                 ...
                                                 if !%i[Sun
                                                        HarshSun].include?(battler.effectiveWeather) && !(battle.pbRandom(100) < 50)
                                                   next
                                                 end
                                                 ...
                                                 battler.setRecycleItem(nil)
                                                 battler.setInitialItem(battler.item) unless battler.initialItem
                                                 battle.pbDisplay(_INTL('¡{1}ha cosechado una baya {2}!',
                                                                        battler.pbThis, battler.itemName))
                                                 battle.pbHideAbilitySplash(battler)
                                                 battler.pbHeldItemTriggerCheck
                                               })
...
Battle::AbilityEffects::EndOfRoundGainItem.add(:PICKUP,
                                               proc { |ability, battler, battle|
                                                 ...
                                                 battle.pbDisplay(_INTL('¡{1} ha encontrado un {2}!', battler.pbThis,
                                                                        battler.itemName))
                                                 battle.pbHideAbilitySplash(battler)
                                                 battler.pbHeldItemTriggerCheck
                                               })
...
Battle::AbilityEffects::OnSwitchIn.add(:AIRLOCK,
                                       proc { |ability, battler, battle, switch_in|
                                         battle.pbShowAbilitySplash(battler)
                                         unless Battle::Scene::USE_ABILITY_SPLASH
                                           battle.pbDisplay(_INTL('¡{1} tiene {2}!', battler.pbThis,
                                                                  battler.abilityName))
                                         end
                                           ...
                                           b.eachMove do |m|
                                             ...
                                             if types.length > 0
                                               ...
                                               next if !Effectiveness.super_effective?(eff) &&
                                                       !%w[OHKO OHKOIce
                                                           OHKOHitsUndergroundTarget].include?(m.function_code)
                                             elsif !%w[OHKO OHKOIce
                                                       OHKOHitsUndergroundTarget].include?(m.function_code)
                                               next
                                             end
                                             found = true
...
Battle::AbilityEffects::OnSwitchIn.add(:ASONECHILLINGNEIGH,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('¡{1} está muy nervioso y no puede comer bayas!',
                                                                battler.pbOpposingTeam))
                                         battle.pbHideAbilitySplash(battler)
...
Battle::AbilityEffects::OnSwitchIn.add(:COMATOSE,
                                       proc { |ability, battler, battle, switch_in|
                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('¡{1} está sumido en un profundo letargo!',
                                                                battler.pbThis))
                                         battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::OnSwitchIn.add(:COMMANDER,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battler.allAllies.each do |b|
                                           ...
                                           next unless b.isSpecies?(:DONDOZO)
                                           next if b.effects[PBEffects::Commander]
                                           next if defined?(b.dynamax?) && b.dynamax?

                                           battle.pbShowAbilitySplash(battler)
                                           battle.pbClearChoice(battler.index)
                                           battle.pbDisplay(_INTL('{1} entra en la boca de {2}!', battler.pbThis,
                                                                  b.pbThis(true)))
                                           ...
                                           %i[ATTACK DEFENSE SPECIAL_ATTACK SPECIAL_DEFENSE SPEED].each do |stat|
                                             ...
                                             showAnim = false if b.pbRaiseStatStage(stat, 2, b, showAnim)
                                           end
                                           battle.pbHideAbilitySplash(battler)
                                           break
                                         end
...
Battle::AbilityEffects::OnSwitchIn.add(:COSTAR,
                                       proc { |ability, battler, battle, switch_in|
                                         battler.allAllies.each do |b|
                                           ...
                                           battle.pbDisplay(_INTL('¡{1} copió los cambios de características de {2}!',
...
Battle::AbilityEffects::OnSwitchIn.add(:CURIOUSMEDICINE,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battler.allAllies.each do |b|
                                           ...
                                           if Battle::Scene::USE_ABILITY_SPLASH
                                             battle.pbDisplay(_INTL(
                                                                '¡Los cambios en las características de {1} han sido restaurados!', b.pbThis
                                                              ))
                                           else
...
Battle::AbilityEffects::OnSwitchIn.add(:EMBODYASPECT,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.pbDisplay(_INTL('¡La {1} usada por {2} brilló fuertemente!', mask,
                                                                battler.pbThis(true)))
                                         battler.pbRaiseStatStageByAbility(:SPEED, 1, battler)
                                       ...
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.allOtherSideBattlers(battler.index).each do |b|
                                           b.eachMove do |m|
                                             ...
                                             if Settings::MECHANICS_GENERATION <= 5 && m.function_code == 'TypeDependsOnUserIVs'
                                               power = 80
                                             end
...
Battle::AbilityEffects::OnSwitchIn.add(:HADRONENGINE,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         if battle.field.terrain == :Electric
                                           battle.pbDisplay(_INTL(
                                                              '¡{1} usó el Terreno Eléctrico para energizar su motor futurista!', battler.pbThis
                                                            ))
                                           battle.pbHideAbilitySplash(battler)
                                         else
                                           battle.pbStartTerrain(battler, :Electric)
                                           battle.pbDisplay(_INTL(
                                                              '¡{1} Electrificó el terreno, para energizar su motor futurista!', battler.pbThis
                                                            ))
                                         end
                                       })
...
Battle::AbilityEffects::OnSwitchIn.add(:HOSPITALITY,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battler.allAllies.each do |b|
                                           ...
                                           b.pbRecoverHP(amt)
                                           battle.pbDisplay(_INTL('{1} bebió todo el matcha que {2} preparó!',
...
Battle::AbilityEffects::OnSwitchIn.add(:INTIMIDATE,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.allOtherSideBattlers(battler.index).each do |b|
                                           ...
                                           check_item = true
                                           if b.hasActiveAbility?(%i[CONTRARY GUARDDOG])
                                       ...
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.allBattlers.each do |b|
                                           if b.hasActiveItem?(:ABILITYSHIELD)
                                             ...
                                             @battle.pbDisplay(_INTL(
                                                                 'La habilidad de {1} está protegida por los efectos de su {2}!', b.pbThis, itemname
                                                               ))
                                             next
                                           end
...
Battle::AbilityEffects::OnSwitchIn.add(:ORICHALCUMPULSE,
                                       proc { |ability, battler, battle, switch_in|
                                         if %i[Sun HarshSun].include?(battler.effectiveWeather)
                                           battle.pbShowAbilitySplash(battler)
                                           battle.pbDisplay(_INTL('¡{1} absorbió luz solar, incrementando su ataque!',
                                                                  battler.pbThis))
                                           battle.pbHideAbilitySplash(battler)
                                         else
                                           battle.pbStartWeatherAbility(:Sun, battler)
                                           battle.pbDisplay(_INTL('¡{1} invocó el sol, incrementando su ataque!',
                                                                  battler.pbThis))
                                         end
                                       })
...
Battle::AbilityEffects::OnSwitchIn.add(:PROTOSYNTHESIS,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         case ability
                                         when :PROTOSYNTHESIS then field_check = %i[Sun
                                                                                    HarshSun].include?(battle.field.weather)
                                         ...
                                         if !field_check && !battler.effects[PBEffects::BoosterEnergy] && battler.effects[PBEffects::ParadoxStat]
                                           battle.pbDisplay(_INTL('¡Los efectos de {2} de {1} se disiparon!',
                                         ...
                                         if highestStat
                                           ...
                                           if field_check
                                             ...
                                             battle.pbDisplay(_INTL("El #{cause} activó {2} de {1}!",
                                                                    battler.pbThis(true), battler.abilityName))
                                           elsif battler.item_id == :BOOSTERENERGY
                                             battler.effects[PBEffects::BoosterEnergy] = true
                                             battle.pbDisplay(_INTL('{1} usó un {2} para activar su {3}!', battler.pbThis, battler.itemName,
                                                                    battler.abilityName))
                                             battler.pbHeldItemTriggered(battler.item)
                                           end
                                           conector = if %i[ATTACK SPATK].include?(highestStat)
                                                        'El'
                                                      elsif %i[DEFENSE SPDEF SPEED].include?(highestStat)
                                                        'La'
                                                      else
                                                        'El'
                                                      end
...
Battle::AbilityEffects::OnSwitchIn.add(:SCREENCLEANER,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         if battler.pbOpposingSide.effects[PBEffects::AuroraVeil] > 0
                                           battler.pbOpposingSide.effects[PBEffects::AuroraVeil] = 0
                                           battle.pbDisplay(_INTL(
                                                              '¡El efecto de Velo Aurora en el {1} se ha disipado!', battler.pbOpposingTeam
                                                            ))
                                         end
                                         if battler.pbOpposingSide.effects[PBEffects::LightScreen] > 0
                                           battler.pbOpposingSide.effects[PBEffects::LightScreen] = 0
                                           battle.pbDisplay(_INTL(
                                                              '¡El efecto de Pantalla de Luz en el {1} se ha disipado!', battler.pbOpposingTeam
                                                            ))
                                         end
                                         if battler.pbOpposingSide.effects[PBEffects::Reflect] > 0
                                           battler.pbOpposingSide.effects[PBEffects::Reflect] = 0
                                           battle.pbDisplay(_INTL('¡El efecto de Reflejo en el {1} se ha disipado!',
                                                                  battler.pbOpposingTeam))
                                         end
                                         if battler.pbOwnSide.effects[PBEffects::AuroraVeil] > 0
                                           battler.pbOwnSide.effects[PBEffects::AuroraVeil] = 0
                                           battle.pbDisplay(_INTL(
                                                              '¡El efecto de Velo Aurora en el {1} se ha disipado!', battler.pbTeam
                                                            ))
                                         end
                                         if battler.pbOwnSide.effects[PBEffects::LightScreen] > 0
                                           battler.pbOwnSide.effects[PBEffects::LightScreen] = 0
                                           battle.pbDisplay(_INTL(
                                                              '¡El efecto de Pantalla de Luz en el {1} se ha disipado!', battler.pbTeam
                                                            ))
                                         end
...
Battle::AbilityEffects::OnSwitchIn.add(:SUPERSWEETSYRUP,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL(
                                                            '¡La cubierta de caramelo de {1} emana un aroma super dulce!', battler.pbThis
                                                          ))
                                         battle.allOtherSideBattlers(battler.index).each do |b|
                                           next if !b.near?(battler) || b.fainted?

                                           if b.itemActive? && !b.hasActiveAbility?(:CONTRARY) && b.effects[PBEffects::Substitute] == 0 && Battle::ItemEffects.triggerStatLossImmunity(b.item, b, :EVASION,
                                                                                                                                                                                       battle, true)
                                             next
                                           end
...
Battle::AbilityEffects::OnSwitchIn.add(:SUPREMEOVERLORD,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('¡{1} ganó la fuerza de sus aliados caídos!',
                                                                battler.pbThis))
...
Battle::AbilityEffects::OnSwitchIn.add(:TABLETSOFRUIN,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('{2} de {1} redujo {4} {3} de todos los Pokémon a su alrededor!', battler.pbThis,
                                                                battler.abilityName, stat_name, conector))
                                         battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::OnSwitchIn.add(:ILLUMINATE,
                                       proc { |ability, battler, battle, switch_in|
                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('{2} de {1} redujo el poder ofensivo de los Pokémon de tipo Siniestro!', battler.pbThis,
                                                                battler.abilityName))
                                         battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::OnSwitchIn.add(:TERAFORMZERO,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         if terrain != :None && battle.field.defaultTerrain == :None
                                           battle.pbShowAbilitySplash(battler) unless showSplash
...
Battle::AbilityEffects::OnSwitchIn.add(:TERAVOLT,
                                       proc { |ability, battler, battle, switch_in|
                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('¡{1} desprende un aura chisporroteante!',
                                                                battler.pbThis))
                                         battle.pbHideAbilitySplash(battler)
                                       })
...
Battle::AbilityEffects::OnSwitchIn.add(:ZEROTOHERO,
                                       proc { |ability, battler, battle, switch_in|
                                         next unless battler.isSpecies?(:PALAFIN)
                                         next if battler.form == 0 || battler.ability_triggered?

                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('¡{1} sufrió una transformación heroíca!',
                                                                battler.pbThis))
                                         battle.pbHideAbilitySplash(battler)
                                         battle.pbSetAbilityTrigger(battler)
                                       })
...
Battle::AbilityEffects::OnSwitchIn.add(:FORECAST,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.pbDisplay(_INTL('¡{1} prefiere el tipo {2}!', battler.pbThis,
                                                                GameData::Type.get(plateType).name))
                                         if plateType == :WATER
                                           battle.pbStartWeatherAbility(:Rain, battler)
                                         ...
                                         elsif %i[ROCK GROUND STEEL].include?(plateType)
                                           battle.pbStartWeatherAbility(:Sandstorm, battler)
...
Battle::AbilityEffects::OnSwitchIn.add(:CRYPTIC,
                                       proc { |ability, battler, battle, switch_in|
                                         next unless $player.has_species?(:UNOWN)

                                         battle.pbShowAbilitySplash(battler)
                                         battle.pbDisplay(_INTL('¡{1} ha recibido fuerza de los unowns de su equipo!',
                                                                battler.pbThis))
                                         battler.pbRaiseStatStageByAbility(:SPECIAL_ATTACK, 1, battler)
                                         battler.pbRaiseStatStageByAbility(:SPEED, 1, battler)
                                         battle.pbHideAbilitySplash(battler)
                                         battler.allAllies.each do |b|
                                           next if !b || !b.near?(battler) || b.fainted?
                                           next unless b.isSpecies?(:UNOWN)

                                           battle.pbShowAbilitySplash(battler)
                                           battle.pbDisplay(_INTL('¡{1} ha recibido fuerza del unown aliado!',
                                                                  battler.pbThis))
                                           battler.pbRaiseStatStageByAbility(:SPECIAL_ATTACK, 1, battler)
                                           battler.pbRaiseStatStageByAbility(:SPEED, 1, battler)
                                           battle.pbHideAbilitySplash(battler)
...
Battle::AbilityEffects::OnSwitchIn.add(:TINTINEO,
                                       proc { |ability, battler, battle, switch_in|
                                         ...
                                         battle.allSameSideBattlers(battler.index).each do |b|
                                           ...
                                           oldStatus = b.status
                                           b.pbCureStatus(Battle::Scene::USE_ABILITY_SPLASH)
                                           next if Battle::Scene::USE_ABILITY_SPLASH
...
Battle::AbilityEffects::ChangeOnBattlerFainting.add(:POWEROFALCHEMY,
                                                    proc { |ability, battler, fainted, battle|
                                                      ...
                                                      next if fainted.ungainableAbility? ||
                                                         %i[POWEROFALCHEMY RECEIVER TRACE
                                                            WONDERGUARD].include?(fainted.ability_id)
                                                      ...
                                                      battle.pbDisplay(_INTL(
                                                                         '¡El Pokémon ha recibido la habilidad {2} de {1}!', fainted.pbThis, fainted.abilityName
                                                                       ))
                                                      battle.pbHideAbilitySplash(battler)
                                                    })
...
Battle::AbilityEffects::OnTerrainChange.add(:MIMICRY,
                                            proc { |ability, battler, battle, ability_changed|
                                              if battle.field.terrain == :None
                                                ...
                                                battle.pbDisplay(_INTL('¡{1} ha cambiado de nuevo a su tipo normal!',
                                                                       battler.pbThis))
                                                battle.pbHideAbilitySplash(battler)
                                              else
                                                ...
                                                if new_type
                                                  ...
                                                  new_type = nil unless type_data
                                                  new_type_name = type_data.name if type_data
                                                end
                                                if new_type
                                                  battle.pbShowAbilitySplash(battler)
                                                  battler.pbChangeTypes(new_type)
                                                  battle.pbDisplay(_INTL('¡El tipo de {1} cambió a tipo {2}!',
                                                                         battler.pbThis, new_type_name))
                                                  battle.pbHideAbilitySplash(battler)
                                                end
                                              end
                                            })
...
Battle::AbilityEffects::OnTerrainChange.add(:QUARKDRIVE,
                                            proc { |ability, battler, battle, switch_in|
                                              Battle::AbilityEffects.triggerOnSwitchIn(ability, battler, battle,
                                                                                       switch_in)
                                            })
                                               ...
                                               proc { |ability, battler, battle, statUps|
                                                 ...
                                                 unless showAnim
                                                   ...
                                                   battle.allOtherSideBattlers(battler.index).each do |b|
                                                     ...
                                                     b.pbItemOpposingStatGainCheck(opposingStatUps) if b.itemActive?
                                                   end
...
Battle::AbilityEffects::OnMoveSuccessCheck.add(:TERASHELL,
                                               proc { |ability, user, target, move, battle|
                                                 next unless target.damageState.terashell

                                                 battle.pbShowAbilitySplash(target)
                                                 battle.pbDisplay(_INTL(
                                                                    '¡{1} hizo brillar su caparazón! Está distorsionando la tabla de tipos!', target.pbThis
                                                                  ))
                                                 battle.pbHideAbilitySplash(target)
                                               })
