...
module System
  ...
  def self.uptime
    ...
    (unscaled_uptime * TurboConfig::SPEED_STAGES[$GameSpeed]) + $SpeedDifference
  end
end
...
module Input
  class << self
    alias turbo_update update unless method_defined?(:turbo_update)
  end
  ...
  def self.update
    ...
    if $CanToggle && TurboConfig::TOGGLE_KEYS.any? do |key|
      trigger?(key)
    end && (!Input.text_input || !trigger?(Input::AUX1))
...
class PokemonSystem
  ...
  attr_accessor :only_speedup_battles, :battle_speed
...
module Input
  def self.update
    ...
    return unless $CanToggle && (trigger?(Input::ALT) || trigger?(Input::AUX1))
...
module Graphics
  class << self
    ...
    def update
      _old_update_turbo
      $buttonframes ||= TurboConfig::ICON_DURATION

      # Mostrar icono si el contador está activo
      return unless $buttonframes < TurboConfig::ICON_DURATION

      if !@boton_turbo || @boton_turbo.disposed?
        @boton_turbo = Sprite.new
        @boton_turbo.z = 999_999
        @boton_turbo.x = 8
        @boton_turbo.y = 8
        set_turbo_bitmap
      elsif @boton_turbo
...
EventHandlers.add(:on_enter_map, :fix_turbo_collision, proc { |_map_id|
  ...
  if !($DEBUG && Input.press?(Input::CTRL)) && $game_player
    ...
    if $game_player.through
      ...
      if $game_player.passable?(player_x, player_y, 0)
