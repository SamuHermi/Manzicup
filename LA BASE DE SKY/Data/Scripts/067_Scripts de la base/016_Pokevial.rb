...
class PokemonGlobalMetadata
  attr_accessor :vial_charges, :max_vial_charges, :vial_locked
end
...
def use_pokevial
  ...
  return unless pbConfirmMessage(_INTL('¿Quieres curar a tu equipo?'))
...
def add_new_vial_charge
  ...
  pbMessage(_INTL(
              "¡Ahora tu Curaportátil puede almacenar {1} carga#{$PokemonGlobal.max_vial_charges > 1 ? 's' : ''}!", $PokemonGlobal.max_vial_charges
            ))
end
...
def remove_vial_charge
  ...
  return unless $PokemonGlobal.max_vial_charges > 1
