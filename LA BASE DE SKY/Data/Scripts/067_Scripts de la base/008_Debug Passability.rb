...
class Debug_Passability
  ...
  def get_event_logical_rect(event)
    ...
    if event.respond_to?(:hitbox_rx) && (event.hitbox_rx > 0 || event.hitbox_ry > 0)
      rx = event.hitbox_rx
      ry = event.hitbox_ry
      rx, ry = ry, rx if event.respond_to?(:hitbox_rotate) && event.hitbox_rotate && [4, 6].include?(event.direction)
    ...
    if event.respond_to?(:hitbox_cx) && (event.hitbox_cx > 0 || event.hitbox_hy > 0)
      cx = event.hitbox_cx
      hy = event.hitbox_hy
      if event.respond_to?(:hitbox_rotate) && event.hitbox_rotate && [4, 6].include?(event.direction)
    ...
    Rect.new(event.x, event.y - bh + 1, bw, bh)
  end
  ...
  def valid?(x, y)
    (x >= 0 and x < $game_map.width and y >= 0 and y < $game_map.height)
  end

  # Method which returns whether a square is an event or not.
  def isEvent?(x, y)
    return false unless SHOW_EVENTS
  ...
  def tileHasTerrainTag?(x, y)
    ...
    return unless terrain_tag.id_number > 0
...
def passability_needs_update?
  ...
  if $passa_last_player_x && $passa_last_player_y
    ...
    return true if player_moved_distance > 3 # Re-renderizar cuando se mueva 3+ casillas
  end
...
class Scene_Map
  def main
    ...
    Graphics.freeze
    disposeSpritesets
    return unless defined?($game_temp.to_title) && $game_temp.to_title
