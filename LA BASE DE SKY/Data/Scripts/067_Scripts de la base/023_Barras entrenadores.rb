if Settings::MOSTRAR_BARRAS_ENTRENADORES # << Make true to use this script, false to disable.
  ...
  module TrainerSensor
    OCULTAR_BARRAS_ENTRENADORES = 65
    BAR_OPACITY = 255 / 8
    SELF_SWITCH = 'A'
  ...
  module TrainerSensor
    ...
    def self.update
      ...
      if @triggered # Que aparezcan deslizándose
        ...
        @created = false if @top.y <= (-BAR_HEIGHT)
      end
    end
  end

  def update_trainer_bars
    return if $game_switches[TrainerSensor::OCULTAR_BARRAS_ENTRENADORES]

    entrenadorMasCercano = RANGE_BARS_TRAINER + 1

    for event in $game_map.events.values
      next unless event.name[/^Trainer\((\d+)\)$/] && event.isOff?(TrainerSensor::SELF_SWITCH)
