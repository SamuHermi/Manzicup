...
class PokemonSummary_Scene
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    limit.times do |i|
      ...
      if move_to_learn
        imagepos.push([_INTL('Graphics/UI/BetterMoveSummary/recuadro'), P4_LEARN_BOX_X,
                       P4_LEARN_BOX_Y])
      end
      if move_to_learn
        imagepos.push([_INTL('Graphics/UI/BetterMoveSummary/help_actionkey'), P4_LEARN_ACTION_KEY_X,
                       P4_LEARN_ACTION_KEY_Y])
      end
  ...
  unless PluginManager.installed?('Modular UI Scenes')
    def pbScene
      @pokemon.play_cry
      loop do
        Graphics.update
  ...
  def showAbilityDescription(pokemon)
    ...
    if @pokemon.male?
      textpos.push([_INTL('♂'), TEXT_GENDER_X, TEXT_GENDER_Y, :left, Color.new(24, 146, 240),
                    Color.new(13, 73, 119)])
    elsif @pokemon.female?
      textpos.push([_INTL('♀'), TEXT_GENDER_X, TEXT_GENDER_Y, :left, Color.new(249, 93, 210),
                    Color.new(128, 20, 90)])
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    ...
    drawTextEx(overlay, ABILITY_DESC_X, ABILITY_DESC_Y, ABILITY_DESC_WIDTH, ABILITY_DESC_HEIGHT, abilitydesc,
               Color.new(64, 64, 64), shadow)
    loop do
      Graphics.update
      Input.update
      pbUpdate
