...
class Game_Temp
  attr_accessor :warned_low_battery, :cue_bgm, :cue_bgm_timer_start, :cue_bgm_delay
end

def pbBatteryLow?
  pstate = System.power_state
  # If it's not discharging, it doesn't matter if it's low
  return false unless pstate[:discharging]
...
EventHandlers.add(:on_player_step_taken, :gain_happiness,
                  proc {
                    $PokemonGlobal.happinessSteps = 0 unless $PokemonGlobal.happinessSteps
...
EventHandlers.add(:on_step_taken, :grass_rustling,
                  proc { |event|
                    ...
                    next if event.respond_to?('name') && GRASS_RUSTLE_EXCLUDED_EVENT_NAMES.any? do |name|
                      event.name[/#{name}/i]
                    end

                    event.each_occupied_tile do |x, y|
                      next unless $map_factory.getTerrainTagFromCoords(event.map.map_id, x, y, true).shows_grass_rustle

                      # Only show animation if it's the player or if the tile is visible on screen
                      if event != $game_player && MUTE_GRASS_RUSTLE_OUT_OF_SIGHT && !$game_map.tile_visible_by_player?(x, y)
                        next
                      end
...
EventHandlers.add(:on_step_taken, :still_water_ripple,
                  proc { |event|
                    ...
                    event.each_occupied_tile do |x, y|
                      next unless $map_factory.getTerrainTagFromCoords(event.map.map_id, x, y, true).shows_water_ripple
...
def pbOnStepTaken(eventTriggered)
  ...
  $PokemonGlobal.stepcount = 0 unless $PokemonGlobal.stepcount
...
EventHandlers.add(:on_player_change_direction, :trigger_encounter,
                  proc {
                    repel_active = ($PokemonGlobal.repel > 0)
                    pbBattleOnStepTaken(repel_active) unless $game_temp.in_menu
                  })

def pbBattleOnStepTaken(repel_active)
  return if $player.able_pokemon_count == 0 && !pbInSafari?
  return unless $PokemonEncounters.encounter_possible_here?

  encounter_type = $PokemonEncounters.encounter_type
  return unless encounter_type
  return unless $PokemonEncounters.encounter_triggered?(encounter_type, repel_active)
...
EventHandlers.add(:on_enter_map, :add_to_trail,
                  proc { |_old_map_id|
                    ...
                    $PokemonGlobal.mapTrail = [] unless $PokemonGlobal.mapTrail
...
def pbFacingTile(direction = nil, event = nil)
  ...
  pbFacingTileRegular(direction, event)
end

# NOTE: Assumes the event is 1x1 tile in size. Only returns one tile.
def pbFacingTileRegular(direction = nil, event = nil)
  event ||= $game_player
  return [0, 0, 0] unless event
  ...
  [$game_map.map_id, x + x_offset, y + y_offset]
end
...
def pbEventCanReachPlayer?(event, player, distance)
  ...
  return false unless pbEventFacesPlayer?(event, player, distance)
  ...
  if real_distance > 0
    real_distance.times do |i|
      return false unless event.can_move_from_coordinate?(event.x + (i * delta_x), event.y + (i * delta_y),
                                                          event.direction)
    end
  end
  true
...
def pbFacingEachOther(event1, event2)
  ...
  pbEventFacesPlayer?(event1, event2, 1) && pbEventFacesPlayer?(event2, event1, 1)
end
...
def pbCueBGM(bgm, seconds, volume = nil, pitch = nil)
  ...
  if !playingBGM || playingBGM.name != bgm.name || playingBGM.pitch != bgm.pitch
    ...
    unless $game_temp.cue_bgm_delay
...
def pbMoveTowardPlayer(event)
  ...
  return unless pbEventCanReachPlayer?(event, $game_player, maxsize)
...
def pbSetEscapePoint
  $PokemonGlobal.escapePoint = [] unless $PokemonGlobal.escapePoint
  xco = $game_player.x
  yco = $game_player.y
...
def pbBuyPrize(item, quantity = 1)
  ...
  pocket = item.pocket
  return false unless $bag.add(item, quantity)
