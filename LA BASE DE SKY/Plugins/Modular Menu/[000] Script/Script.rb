...
module ModularMenu
  ...
  def self.add_entry(ref, name, icon, &block)
    ...
    @@available[ref] = true unless @@available.keys.include?(ref)
    @@indexes[ref] = @@index
    @@index += 1
  end
  ...
  #-----------------------------------------------------------------------------
  def self.icon(ref)
    "Graphics/Icons/#{@@menuEntry[ref][1]}"
  end
  ...
  def self.available?(ref)
    @@available[ref].is_a?(Proc) ? @@available[ref].call : @@available[ref]
  end
...
class PokemonPauseMenu_Scene
  attr_accessor :index, :entries, :endscene, :close, :hidden
  ...
  def pbStartScene
    ...
    when 1
      ...
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_hermi'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 2
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_iria'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_iria'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 4
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_brais'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_brais'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 5
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_isa'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_isa'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 6
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_samer'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_samer'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 7
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_rodri'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_rodri'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 8
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_bra'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_bra'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 9
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_ana'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_ana'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 10
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_pablo'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_pablo'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 11
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_sabo'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_sabo'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    when 12
      @selRoute = 'Graphics/Pictures/PauseMenu/sel_nerea'
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg_nerea'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    else
      @sprites['background'].bitmap.blt(0, 0, pbBitmap('Graphics/Pictures/PauseMenu/bg'),
                                        Rect.new(0, 0, Graphics.width, Graphics.height))
    end
    bmp = pbBitmap('Graphics/Pictures/Common/scrollbar_bg')
    @sprites['background'].bitmap.blt(Graphics.width - 28, (Graphics.height - bmp.height) / 2, bmp,
  ...
  def refresh
    ...
    for i in 0...@entries.length
      ...
      for j in 0...2
        ...
        pbDrawOutlineText(@sprites["#{i}"].bitmap, 66 + j * bmp.width / 2, 18, 136, 48, text, Color.new(255, 255, 255),
                          Color.new(64, 64, 64), 1)
      end
  ...
  def update
    ...
    for i in 0...@entries.length
      ...
      elsif @sprites["#{i}"].opacity > 128
        @sprites["#{i}"].opacity -= 15
      end
      if @index == i
        @sprites["#{i}"].tone.gray -= 51 if @sprites["#{i}"].tone.gray > 0
      elsif @sprites["#{i}"].tone.gray < 255
        @sprites["#{i}"].tone.gray += 51
      end
    end
    # sets the index
    return unless @oldindex != @index
