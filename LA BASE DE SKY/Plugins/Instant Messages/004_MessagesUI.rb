...
def pbReceiveIM(conversation_id, silent = false)
    ...
    unless silent || convo.important
        if defined?(pbAddItemSplash)
            pbAddGenericSplash(_INTL("¡Mensaje nuevo!"), style: :Message)
        else
            pbSEPlay(se)
            pbMessage(_INTL("¡Tienes una notificación!"))
        end
    end
    if convo.important
        pbSEPlay(se)
...
class InstantMessagesMenu_Scene
    ...
    def pbStartScene
        ...
        if @sprites["itemlist"].item_max == 0
            if @filter
                textpos.push([_INTL("No se han encontrado mensajes."), Graphics.width / 2, Graphics.height / 2 - 12, 2, @base_color, @shadow_color])
            else
                textpos.push([_INTL("No tienes mensajes."), Graphics.width / 2, Graphics.height / 2 - 12, 2, @base_color, @shadow_color])
            end
        end
        ...
        loop do
            ...
            if Input.trigger?(Input::BACK)
                if pbCheckForImportant
                    pbMessage(_INTL("¡Deberías revisar tus conversaciones importantes!"))
                else
                    pbPlayCloseMenuSE
                    break
                end
                ...
                commands = [_INTL("Organizar por nuevos"),_INTL("No leídos primeros"),_INTL("Ordenar alfabéticamente")]
...
class InstantMessages_Scene
    ...
    def pbStartScene
        ...
        @old_texts.length.times do |i|
            ...
            when :Leave
                text_to_show = _INTL("{1} salió.", @members[@old_texts[i][2]].name)
            when :Enter
            ...
            if i == @old_texts.length - 1
                if @max_texts > 0
                    @sprites["unreaddivider"] = InstantMessageDivider.new(0, 0, _INTL("No leídos"), true, self, @viewport)
        ...
        if @max_texts > 0
            ...
            @max_texts.times do |i|
                ...
                when :Leave
                    text_to_show = _INTL("{1} salió.", @members[@texts[i][2]].name)
                when :Enter
                    text_to_show = _INTL("{1} fue añadido al grupo.", @members[@texts[i][2]].name)
                when :GroupName
                    @texts_code_to_execute[i] = [@texts[i][1], @texts[i][2]]
                    text_to_show = _INTL("El nombre del grupo a cambiado a: {1}", @texts[i][2] || @group.original_title)
                when :Picture
    ...
    def check_next_text
        ...
        if @show_index == 0 # First message of the chat
            ...
            typing_text = _INTL("{1} está escribiendo", @members[@texts[@show_index][0]].name)
    ...
    def pbConvertTimeStamp(instance)
        ...
        if diff < 86400 && now.day == instance.day # now.strftime("%m/%d/%Y") == instance.strftime("%m/%d/%Y")
            val = _INTL("Hoy a las {1}", instance.strftime("%I:%M %p"))
        elsif diff < 86400 * 2 && (now - 86400).day == instance.day
            val = _INTL("Ayer a las {1}", instance.strftime("%I:%M %p"))
        else 
            days = (diff / 86400).floor
            if days >= 7
                val = _INTL("Hace {1} días a las {2}", days.to_s_formatted, instance.strftime("%I:%M %p"))
            else
                case instance.wday
                when 0
                    wd = "Sun"
                ...
                val = _INTL("{1} a las {2}", wd, instance.strftime("%I:%M %p"))
            end
        end
        return val
    end
