...
class PokemonGlobalMetadata
  attr_accessor :tmCompatibilityRandom, :randomMoves, :randomGens, :enable_random_moves, :progressive_random,
                :enable_random_tm_compat, :enable_random_evolutions, :enable_random_evolutions_similar_bst, :enable_random_abilities
end
...
def add_or_remove_random_gen(gen = nil)
  ...
  $PokemonGlobal.randomGens = [] unless $PokemonGlobal.randomGens
...
def enable_random
  ...
  if $PokemonGlobal.enable_random_evolutions.nil?
    $PokemonGlobal.enable_random_evolutions = RandomizedChallenge::RANDOM_EVOLUTIONS_DEFAULT_VALUE
  end
  if $PokemonGlobal.enable_random_evolutions_similar_bst.nil?
    $PokemonGlobal.enable_random_evolutions_similar_bst = RandomizedChallenge::RANDOM_EVOLUTIONS_SIMILAR_BST_DEFAULT_VALUE
  end
...
class Pokemon
  ...
  def getMaxBSTCap
    return 300 if $player.badge_count <= 1
  ...
  def initialize(species, level = 5, owner = $player, withMoves = true, recheck_form = true)
    if $game_switches && $game_switches[RandomizedChallenge::Switch] && !RandomizedChallenge::UNRANDOMIZABLE_TRAINERS.include?(owner.trainer_type)
  ...
  def getMoveList
    ...
    if $game_switches && $game_switches[RandomizedChallenge::Switch] && $PokemonGlobal.enable_random_moves
      $PokemonGlobal.randomMoves = {} unless $PokemonGlobal.randomMoves
      return $PokemonGlobal.randomMoves[@species] if $PokemonGlobal.randomMoves[@species]
  ...
  def compatible_with_move?(move_id)
    ...
    $PokemonGlobal.tmCompatibilityRandom = {} unless $PokemonGlobal.tmCompatibilityRandom
    if $PokemonGlobal.tmCompatibilityRandom && $PokemonGlobal.tmCompatibilityRandom[species] && $PokemonGlobal.tmCompatibilityRandom[species].detect do |item|
      item[0] == move_id
    end
      $PokemonGlobal.tmCompatibilityRandom[species].any? { |item| item[0] == move_id && item[1] == true }
    elsif !$PokemonGlobal.tmCompatibilityRandom[species]
      $PokemonGlobal.tmCompatibilityRandom[species] = []
      if rand(2) == 1
        $PokemonGlobal.tmCompatibilityRandom[species].push([move_id, true])
        true
      else
        $PokemonGlobal.tmCompatibilityRandom[species].push([move_id, false])
        false
      end
    elsif rand(2) == 1
      $PokemonGlobal.tmCompatibilityRandom[species].push([move_id, true])
  ...
  def get_random_evo(current_species, new_species)
    ...
    return species_list[0] unless are_random_evolutions_similar_bst_on
...
def generarInicialesRandom
  inicial_1 = RandomizedChallenge::ListaStartersRandomizado.sample

  inicial_2 = inicial_1
  loop do
    inicial_2 = RandomizedChallenge::ListaStartersRandomizado.sample
    break if inicial_1 != inicial_2
  end

  inicial_3 = inicial_1
  loop do
    inicial_3 = RandomizedChallenge::ListaStartersRandomizado.sample
