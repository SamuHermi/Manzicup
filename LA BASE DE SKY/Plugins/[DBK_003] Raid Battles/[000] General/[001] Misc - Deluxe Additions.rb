  ...
  proc { |battle, idxBattler, idxTarget, trigger|
	...
	when "RoundStartAttack_foe"
	  ...
	  if foe.turnCount % 2 == 0 && battle.zMove[1][0] == -2
		...
		battle.pbDisplay(_INTL("¡{1} recuperó su Poder-Z con un subidón de energía!", foe.pbThis))
		battle.zMove[1][0] = -1
      end
	#---------------------------------------------------------------------
	...
	when "RoundEnd_player", "RoundEnd_ally"
	  ...
	  battle.pbParty(0).each do |pkmn|
        ...
        battle.pbDisplayPaused(_INTL("¡{1} se recuperó!\n¡Volverá en el próximo turno!", pkmn.name))
	  end
