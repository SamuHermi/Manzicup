...
class Battle
  ...
  def pbRaidChangeTurnCount(battler, amt)
	...
	pbDisplayPaused(_INTL("¡La energía que rodea a {1} se ha descontrolado!", battler.pbThis(true)))
    pbDisplay(_INTL("¡Te ha arrastrado fuera del nido!"))
  ...
  def pbRaidChangeKOCount(battler, amt, done_fainting)
	...
	when 0 then pbDisplayPaused(_INTL("¡La energía que rodea a {1} se ha descontrolado!", battler.pbThis(true)))
	when 1 then pbDisplay(_INTL("La energía que rodea a {1} te hace tambalear!", battler.pbThis(true)))
	else        pbDisplay(_INTL("¡La energía que rodea a {1} aumenta!", battler.pbThis(true)))
	end
  ...
  def pbStorePokemon(pkmn)
    if pkmn.immunities.include?(:RAIDBOSS) && @raidStyleCapture && !@caughtPokemon.empty?
      ...
      if pbInRaidAdventure?
        ...
        pbDisplay(_INTL("Has capturado a {1}!", pkmn.name))
      else
        pkmn.heal
        pkmn.reset_moves
        pkmn.calc_stats
