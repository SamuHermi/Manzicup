...
class Battle
  ...
  def pbRaidResetDrops(battler)
	...
	pbDisplay(_INTL("¡{1} se libró de los efectos negativos!", battler.pbThis))
  ...
  def pbRaidResetBoosts(battler)
    @scene.pbAnimateExtraAction(battler.index)
	pbDisplay(_INTL("¡{1} anuló los cambios en las características y habilidades!", battler.pbThis))
  ...
  def pbRaidDrainCheer(battler)
    @scene.pbAnimateExtraAction(battler.index)
	pbDisplay(_INTL("¡{1} redujo la eficacia de los ánimos!", battler.pbThis))
