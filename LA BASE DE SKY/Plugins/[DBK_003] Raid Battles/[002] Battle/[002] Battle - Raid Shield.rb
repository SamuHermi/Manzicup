...
class Battle::Battler
  ...
  def pbSuccessCheckAgainstTarget(move, user, target, targets)
    if target.hasRaidShield? && move.statusMove?
      @battle.pbDisplay(_INTL("¡La barrera protege a {1}!", target.pbThis(true)))
      return false
    end
  ...
  def startRaidShield(shield_hp = 0)
	...
	@battle.pbDisplay(_INTL("¡Una corriente de energía comienza a rodear a {1}!", pbThis(true)))
	...
	@battle.pbDisplay(_INTL("¡Una barrera misteriosa aparece delante de {1}!", pbThis(true)))
	pbCureStatus
  ...
  def setRaidShieldHP(amt, user = nil)
	...
	return if @battle.pbAllFainted? || @battle.decision > 0
	@battle.pbDisplay(_INTL("¡La barrera misteriosa ha desaparecido!"))
    oldhp = @hp
    @hp -= @totalhp / 8
	@hp = 1 if @hp <= 1
