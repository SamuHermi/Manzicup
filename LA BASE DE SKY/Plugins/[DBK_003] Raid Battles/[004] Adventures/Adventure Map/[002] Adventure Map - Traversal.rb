...
class AdventureMapScene
  ...
  def pbProcessPlayerTile(dir)
    ...
    if !dir || dirs.empty?
      pbMessage(_INTL("¡Es un camino sin salida!\n¡No tienes más opción que dar media vuelta!")) { pbUpdate }
      @adventure.outcome = 4
      pbHideUI(true)
    end
    return dir
  ...
  def pbMovePlayer(dir = 0)
    ...
    loop do
      ...
      if Input.press?(Input::CTRL) && $DEBUG
        @adventure.outcome = 4 if pbConfirmMessageSerious(_INTL("¿Quieres salir del nido?"))
      end
  ...
  def pbSelectRoute(dir, dirs)
    ...
    loop do
      ...
      if Input.trigger?(Input::USE) && 
         pbConfirmMessage(_INTL("¿Seguro que quieres seguir este camino?")) { pbUpdate }
        pbUpdatePokemon(true)
        pbHideUI
        break
      #-------------------------------------------------------------------------
      ...
      # Open command menu.
        ...
        cmd = 0
        commands = [_INTL("Ver mapa")]
        cmd_ids = [:view]
        if !@adventure.playtesting
          commands.push(_INTL("Ver equipo"))
          cmd_ids.push(:summary)
        end
        if @adventure.endlessMode?
          record = $PokemonGlobal.raid_adventure_records(@adventure.style)
          if record && !record.empty?
            commands.push(_INTL("Ver récord"))
            cmd_ids.push(:record)
          end
        end
        commands.push(_INTL("Terminar aventura"))
        cmd_ids.push(:abandon)
        loop do
          cmd = pbMessage(_INTL("¿Qué te gustaría hacer?"), commands, -1, nil, 0) { pbUpdate }
          if cmd < 0
            pbPlayCancelSE
            break
          end
          ...
          when :abandon   # End Adventure
            if @adventure.playtesting
              msg = _INTL("¿Terminar la prueba?")
            else
              adventure_name = GameData::RaidType.get(@adventure.style).lair_name
              msg = _INTL("¿Terminar tu {1}?\nCualquier Pokémon capturado y recompensas conseguidas se perderán.", adventure_name)
            end
    ...
    loop do
        ...
        if teleporter
          if @cursor_tile.isTile?(:Crossroad) && pbCursorReact?
            if @cursor_tile.visited? && @cursor_tile.coords != @player_tile.coords
              if pbConfirmMessage(_INTL("¿Teletransportarse a esta casilla?")) { pbUpdate }
              ...
              pbMessage(_INTL("¡Aún no has estado en esa casilla!")) { pbUpdate }
            end
          else
            pbPlayBuzzerSE
          end
      ...
      # Moves the player to a new crossroad tile.
        ...
        if @cursor_tile.isTile?(:Crossroad)
          if pbConfirmMessage(_INTL("¿Moverse a esta casilla?"))
            @player_tile = @cursor_tile
            @player.x = @player_tile.x
            @player.y = @player_tile.y
            @player_tile.make_visited
          ...
          pbMessage(_INTL("El jugador solo puede moverse a casillas {1}.", tileName))
        end
      end
    end
  end
