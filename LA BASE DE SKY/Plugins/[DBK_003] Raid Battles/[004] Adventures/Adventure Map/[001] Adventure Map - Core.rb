...
class AdventureMapScene
  ...
  def pbUpdateRouteArrows(dir, dirs)
    ...
    pbDrawTextPositions(overlay, [
      [_INTL("¡Elige tu camino!"), Graphics.width / 2, Graphics.height - 60, :center, Color.white, Color.black, :outline]
    ])
  ...
  def pbUpdateControls(mode)
    ...
    when :teleporting  # Controls for Teleporter selection.
      controls = [[_INTL("[USAR] Seleccionar casilla"), 4,  8, :left, Color.white, Color.black, :outline]]
    when :moving       # Controls for player movement.
      images = [[@path + "fast_travel", 0, Graphics.height - 32, 0, 0, 176, 32]]
      controls = [[_INTL("ACCIÓN: Viaje rápido"), 4, Graphics.height - 22, :left, Color.white, Color.black]]
      if $DEBUG
        images.push([@path + "fast_travel", Graphics.width - 176, Graphics.height - 32, 0, 32, 176, 32])
        controls.push([_INTL("CTRL: Huida rápida rápida"), Graphics.width - 8, Graphics.height - 22, :right, Color.white, Color.black])
      end
      pbDrawImagePositions(overlay, images)
    when :selecting    # Controls for route selection.
      colors = [Color.white, Color.new(64, 64, 64)]
      controls = [
        [_INTL("FLECHAS: Seleccionar"),  4,                  Graphics.height - 22, :left,   *colors],
        [_INTL("USAR: Confirmar"),    Graphics.width / 2, Graphics.height - 22, :center, *colors],
        [_INTL("ACCIÓN: Opciones"), Graphics.width - 4, Graphics.height - 22, :right,  *colors]
      ]
    when :viewing      # Controls for map scrolling.
      text_display = [:left, Color.white, Color.black, :outline]
      if @adventure.playtesting
        controls = [
          [_INTL("[ACCIÓN]"),        4,  8, *text_display],
          [_INTL("[SALTAR]"),          4, 28, *text_display],
          [_INTL("[ATRÁS]"),          4, 48, *text_display],
          [_INTL("Alternar cuadrícula"),    82,  8, *text_display],
          [_INTL("Cambiar cursor"),   82, 28, *text_display],
          [_INTL("Regresar"),         82, 48, *text_display]
        ]
      else
        controls = [
          [_INTL("[USAR]"),           4,  8, *text_display],
          [_INTL("[ACCIÓN]"),        4, 28, *text_display],
          [_INTL("[SALTAR]"),          4, 48, *text_display],
          [_INTL("[ATRÁS]"),          4, 68, *text_display],
          [_INTL("Cambiar Pokémon"), 82,  8, *text_display],
          [_INTL("Alternar cuadrícula"),    82, 28, *text_display],
          [_INTL("Cambiar cursor"),   82, 48, *text_display],
          [_INTL("Volver"),         82, 68, *text_display]
        ]
        if $DEBUG
          controls.push(
            [_INTL("[CTRL]"),        4, 88, *text_display],
            [_INTL("Mover jugador"),  82, 88, *text_display]
          )
        end
      end
    end
  ...
  def pbUpdateCursor(snapToTile = false)
    ...
    if cursor_react
      ...
      when :Warp
        ...
        tile_name += sprintf(" a %d, %d", *coords)
      when :Switch
      ...
      if @cursor_tile.toggled?
        ...
        desc = _INTL("Esta casilla está deshabilitada actualmente. Actívala cambiando una casilla {1} a la posición 'ON'.", switch)
      end
  ...
  def pbMapIntro(showTitle = true)
    ...
    pbPauseScene
    @ui_sprites["title"].visible = false
    pbMessage(_INTL("\\se[{1}]¡Hay una fuerte reacción de tipo {2} proveniente del interior del nido!\\wtnp[30]", cryFile, typeName)) { pbUpdate }
  ...
  def pbResetLair
    ...
    pbPauseScene(0.5)
    pbMessage(_INTL("¡Encontraste un pasadizo que lleva aún más profundo en la guarida!")) { pbUpdate }
    if !pbConfirmMessage(_INTL("¿Quieres continuar hacia el siguiente piso?")) { pbUpdate }
      @adventure.outcome = 1
      return
    end
    pbFadeOutIn {
  ...
  def pbEndScene
    ...
    if !@adventure.playtesting && (@adventure.endlessMode? || @adventure.outcome != 1)
      adventure_name = GameData::RaidType.get(@adventure.style).lair_name
      pbMessage(_INTL("¡Tu {1} ha terminado!", adventure_name)) { pbUpdate }
    end
    pbPauseScene(0.5)
