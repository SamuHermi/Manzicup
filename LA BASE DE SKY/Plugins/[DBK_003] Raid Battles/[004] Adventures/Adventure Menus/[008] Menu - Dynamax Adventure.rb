...
class AdventureDynamaxbox < AdventureDataboxCore
  ...
  def refresh
    ...
    textpos = [
      ...
      [_INTL("PS Dinamax"), 235, 46, :center, base, shadow, outline]
    ]
...
class AdventureMenuScene
  ...
  def pbDynamaxMenu
    ...
    overlay.clear
	textpos = [[_INTL("Aumentando el nivel Dinamax..."), 337, 12, :center, BASE_COLOR, SHADOW_COLOR]]
	...
	if party_select.length == PARTY_SIZE
	  pbMessage(_INTL("¡El nivel Dinamax no puede aumentar más!"))
	  overlay.clear
	else
	  pbPauseScene(0.5)
	  pbMessage(_INTL("¡El nivel Dinamax ha aumentado!"))
	  overlay.clear
	  buttonY = 156
	  imagepos = [
	  ...
	  textpos = [
	  [_INTL("El nivel Dinamax ha aumentado!"), 337, 12, :center, BASE_COLOR, SHADOW_COLOR],
		[_INTL("Info"), 56, buttonY + 12, :left, BASE_COLOR, SHADOW_COLOR, :outline],
		[_INTL("Salir"), 56, buttonY + 52, :left, BASE_COLOR, SHADOW_COLOR, :outline]
	  ]
      pbDrawTextPositions(overlay, textpos)
	  loop do
	    Input.update
