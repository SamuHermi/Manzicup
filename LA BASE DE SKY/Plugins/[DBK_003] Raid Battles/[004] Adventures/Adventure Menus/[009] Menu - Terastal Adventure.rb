    ...
    loop do
      ...
      # Selects a party Pokemon or Tera type, depending on selectionMode.
        ...
        when 0 # Selects a party Pokemon.
          pbPlayDecisionSE
          overlay.clear
          textpos[1][0] = _INTL("Elige el Teratipo de {1}.", pkmn.name)
          pbDrawTextPositions(overlay, textpos)
          TYPE_LIST_SIZE.times { |i| @sprites["type_#{i}"].selected = (i == idxType) }
          selectionMode = 1
        when 1 # Selects a Tera type to give.
		  ...
		  if pkmn.has_forced_tera_type?
		    pbMessage("\\se[GUI sel buzzer]" + _INTL("¡No se puede cambiar el Teratipo de {1}!", pkmn.name))
          elsif pkmn.tera_type == types[idxType]
            pbMessage("\\se[GUI sel buzzer]" + _INTL("{1} ya tiene el Teratipo {2}!", pkmn.name, typeName))
          elsif pbConfirmMessage(_INTL("Cambiar el Teratipo de {1} a {2}?", pkmn.name, typeName))
            pkmn.tera_type = types[idxType]
            @sprites["party_#{idxPkmn}"].refresh
			pbMessage("\\se[]" + _INTL("El Teratipo de {1} ahora es {2}!", pkmn.name, typeName) + "\\se[Pkmn move learnt]")
