...
class AdventureMenuScene
  ...
  def pbGenerateRental(species_list)
    ...
    owner = Pokemon::Owner.new(0, _INTL("PRÉSTAMO"), 2, $player.language)
    pkmn = Pokemon.new(species, level, owner)
    pkmn.setRaidRentalAttributes(@style)
    pkmn.obtain_text = _INTL("Préstamo.")
  ...
  def pbRentalsMenu
    ...
    textpos = [
      ...
      [_INTL("Crea un equipo de {1} Pokémon de préstamo.", remainder), 337, 12, :center, BASE_COLOR, SHADOW_COLOR]
    ]
    ...
    until new_party.length == PARTY_SIZE
      ...
      # Exits the menu and prematurely ends the Adventure.
        ...
        break if pbConfirmMessage(_INTL("¿Salir y terminar?", adventure_name))
      #-------------------------------------------------------------------------
      # USE KEY
      #-------------------------------------------------------------------------
      # Selects a rental Pokemon and opens the command menu.
        ...
        loop do
          ...
          case cmd
          when 0 # Select
            if pbConfirmMessage(_INTL("Añadir {1} al equipo de préstamo?", pkmn.name))
              ...
              pbMessage("\\se[#{cryFile}]" + _INTL("¡{1} fue añadido al equipo de préstamo!\\wtnp[30]", pkmn.name))
              if textpos.length < 3
                textpos.push([_INTL("Info"), 56, Graphics.height - 20, :left, BASE_COLOR, SHADOW_COLOR, :outline])
              end
              if new_party.length < PARTY_SIZE
                ...
                overlay.clear
				remainder -= 1
                textpos[1][0] = _INTL("Elige {1} Pokémon más.", remainder)
