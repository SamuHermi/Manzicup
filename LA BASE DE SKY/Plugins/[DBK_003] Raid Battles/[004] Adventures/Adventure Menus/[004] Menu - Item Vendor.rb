...
class AdventureMenuScene
  ...
  def pbItemVendorMenu
    ...
    @sprites["window"].text = _INTL("{1} lleva:\n{2}.", pkmn.name, ((pkmn.hasItem?) ? pkmn.item.name : "Nada"))
    overlay = @sprites["overlay"].bitmap
    overlay.clear
    textpos = [
      [_INTL("EQUIPO DE PRÉSTAMO"), 79, 8, :center, BASE_COLOR, SHADOW_COLOR, :outline],
      [_INTL("Elige a un Pokémon al que equipar."), 337, 12, :center, BASE_COLOR, SHADOW_COLOR],
    ...
    loop do
      ...
      # Exits the menu or returns to party selection, depending on selectionMode.
        ...
        when 1 # Returns to party selection.
          pbPlayCancelSE
          overlay.clear
          textpos[1][0] = _INTL("Elige un Pokémon al que equipar.")
          pbDrawTextPositions(overlay, textpos)
      ...
      # Selects a party Pokemon or an item, depending on selectionMode.
        ...
        when 0 # Selects a party Pokemon.
          pbPlayDecisionSE
          overlay.clear
          textpos[1][0] = _INTL("Elige un objeto para {1}.", pkmn.name)
          pbDrawTextPositions(overlay, textpos)
	      @sprites["descbox"].y -= 44
        ...
        when 1 # Selects an item.
          ...
          if pbConfirmMessage(_INTL("¿Darle a {1} el {2}?", pkmn.name, itemPortion))
		    if pkmn.item_id == items[idxItem]
			  pbMessage("\\se[GUI sel buzzer]" + _INTL("{1} ya lleva ese objeto!", pkmn.name))
            elsif !pkmn.hasItem? || 
			   pbConfirmMessage(_INTL("{1} lleva un {2}...\n¿Quieres cambiar los objetos?", pkmn.name, pkmn.item.portion_name))
			  pbMessage("\\se[]" + _INTL("{1} recibió {2}.", pkmn.name, itemPortion) + "\\se[Pkmn move learnt]")
