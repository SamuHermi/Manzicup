...
class AdventureMovebox < Sprite
  ...
  def refresh
    ...
    textpos = [
      ...
      [_INTL("Potencia: {1}", power), 154, 54, :left, base, shadow],
      [_INTL("Prec: {1}", accuracy), 254, 54, :left, base, shadow]
    ]
...
class AdventureMenuScene
  ...
  def pbMoveTutorMenu
	...
	if moves[idxPkmn].empty?
      @sprites["window"].text = _INTL("{1} no puede aprendar nada.", pkmn.name)
	else
	  @sprites["window"].text = _INTL("Enseña a {1} uno de los movimientos.", pkmn.name)
	end
    overlay = @sprites["overlay"].bitmap
    overlay.clear
    textpos = [
      [_INTL("EQUIPO DE PRÉSTAMO"), 79, 8, :center, BASE_COLOR, SHADOW_COLOR, :outline],
      [_INTL("Elige un Pokémon al que enseñar."), 337, 12, :center, BASE_COLOR, SHADOW_COLOR],
    ...
    loop do
      ...
      # Exits the menu or returns to party selection, depending on selectionMode.
        ...
        when 1 # Returns to party selection.
          pbPlayCancelSE
          overlay.clear
          textpos[1][0] = _INTL("Elige un Pokémon al que enseñar.")
          pbDrawTextPositions(overlay, textpos)
      ...
      # Selects a party Pokemon or a move, depending on selectionMode.
        ...
        when 0 # Selects a party Pokemon.
          if moves[idxPkmn].empty?
            ...
            pbPlayDecisionSE
            overlay.clear
            textpos[1][0] = _INTL("Enseña a {1} un movimiento.", pkmn.name)
            pbDrawTextPositions(overlay, textpos)
            MOVE_LIST_SIZE.times { |i| @sprites["move_#{i}"].selected = (i == idxMove) }
            @sprites["window"].text = @sprites["move_#{idxMove}"].move.description
            selectionMode = 1
        ...
        when 1 # Selects a move.
          ...
          if pbConfirmMessage(_INTL("Enseñar {2} a {1}?", pkmn.name, moveName))
            move_index = -1
            ...
            if move_index >= 0
              ...
              if old_move
                pbMessage(_INTL("1, 2, y...\\wt[16] ...\\wt[16] ...\\wt[16] Puff!") + "\\se[Battle ball drop]\1")
                pbMessage(_INTL("{1} olvidó {2}.\\nY..." + "\1", pkmn.name, old_move.name))
              end
              ...
              pbMessage("\\se[]" + _INTL("{1} aprendió {2}!", pkmn.name, moveName) + "\\se[Pkmn move learnt]")
