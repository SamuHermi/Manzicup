...
class AdventureMenuScene
  ...
  def pbSpoilsMenu(pokemon = nil)
    ...
    overlay.clear
	heading = (pokemon) ? _INTL("Cofre del tesoro") : _INTL("Recompensas")
	...
	if pokemon.nil?
	  if pbRaidAdventureState.outcome == 3
	    pbMessage(_INTL("Conseguiste salvar algo de lo que habías conseguido..."))
	  end
	else
	...
	loop do
      ...
      #-------------------------------------------------------------------------
	    ...
	    if pokemon.nil?
		  ...
		  when 1 then pbMessage(_INTL("Has guardado todo lo conseguido en la bolsa."))
		  when 3 then pbMessage(_INTL("Has guardado en la bolsa todo lo posible."))
		  end
		else
		  pbMessage(_INTL("Has guardado lo que has conseguido en este cofre."))
	    end
