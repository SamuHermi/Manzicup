...
class AdventureMenuScene
  ...
  def pbRewardMenu
    ...
    overlay.clear
    textpos = [
      [_INTL("Pokémon capturado"), 79, 8, :center, BASE_COLOR, SHADOW_COLOR, :outline],
      [_INTL("Elige un Pokémon con el que quedarte."), 337, 12, :center, BASE_COLOR, SHADOW_COLOR],
    ...
    loop do
      ...
      #-------------------------------------------------------------------------
        ...
        break if pbConfirmMessage(_INTL("¿Salir sin quedarte con un Pokémon?"))
      #-------------------------------------------------------------------------
      # USE KEY
      #-------------------------------------------------------------------------
        ...
        if pbConfirmMessage(_INTL("¿Quieres quedarte con {1}?", pkmn.name)) { pbUpdate }
          textpos = [textpos.first]
          if pbBoxesFull?
            pbMessage(_INTL("No tienes sitio para más Pokémon!") + "\1")
            pbMessage(_INTL("¡Las cajas están llenas!"))
            overlay.clear
            pbDrawTextPositions(overlay, textpos)
            ...
            if should_show_pokedex
              pbMessage(_INTL("Los datos de {1} se han registrado en la Pokédex.", pkmn.speciesName)) { pbUpdate }
              pbFadeOutIn {
            ...
            if $PokemonSystem.givenicknames == 0
              ...
              if pbConfirmMessage(_INTL("¿Quieres ponerle un mote a {1}?", species_name)) { pbUpdate }
            ...
            pbMessage(_INTL("¡{1} se ha enviado a la caja \"{2}\"!", pkmn.name, box_name)) { pbUpdate }
            pbMessage(_INTL("Has devuelto los Pokémon capturados y del equipo de préstamo.")) { pbUpdate }
          end
          break
        end
      end
