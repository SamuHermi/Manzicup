...
class Battle
  ...
  def pbEndOfRoundPhase
    ...
    [@player, @opponent].each_with_index do |trainers, side|
	  ...
	  @cheerLevel[side].length.times do |i|
		...
		if newLvl > 0
		  if newLvl < 3
		    pbDisplay(_INTL("¡El nível de ánimo de {1} está aumentando!", name))
		  else
		    pbDisplay(_INTL("¡El nivel de ánimo de {1} está a tope!", name))
		  end
  ...
  def pbEOREndSideEffects(side, priority)
    ...
    pbEORCountDownSideEffect(side, PBEffects::CheerOffense1,
                             _INTL("¡La inspiración para darlo todo de {1} ha desaparecido!", @battlers[side].pbTeam))
	pbEORCountDownSideEffect(side, PBEffects::CheerOffense2,
                             _INTL("¡La inspiración para atacar con más fuerza de {1} ha desaparecido!", @battlers[side].pbTeam))						 
	pbEORCountDownSideEffect(side, PBEffects::CheerOffense3,
                             _INTL("¡La inspiración para romper barreras de {1}ha desaparecido!", @battlers[side].pbTeam))
  pbEORCountDownSideEffect(side, PBEffects::CheerDefense1,
                             _INTL("¡La inspiración para aguantar de {1} ha desaparecido!", @battlers[side].pbTeam))
	pbEORCountDownSideEffect(side, PBEffects::CheerDefense2,
                             _INTL("¡La inspiración para ignorar golpes críticos y efectos de {1} ha desaparecido!", @battlers[side].pbTeam))
	pbEORCountDownSideEffect(side, PBEffects::CheerDefense3,
                             _INTL("¡La inspiración para aguantar golpes de {1} ha desaparecido!", @battlers[side].pbTeam))
  end
  ...
  def hasCheer?(idxBattler = 0, showMessages = false)
    if !canCheer?(idxBattler)
	  pbDisplay(_INTL("No se puede animar.")) if showMessages
	  return false
	end
	if allOwnedByTrainer(idxBattler).any? { |b| @choices[b.index][0] == :Cheer }
	  pbDisplay(_INTL("Ya has animado este turno.")) if showMessages
	  return false
	end
	if pbOwnedByPlayer?(idxBattler) && @scene.sprites["cheerWindow"].cheers.empty?
	  pbDisplay(_INTL("No se te ocurre ninguna palabra de ánimo.")) if showMessages
	  return false
	end
	return true
  end
  ...
  def pbCheer(idxBattler)
    ...
    if Battle::Cheer.trigger(cheer.id, side, owner, battler, self)
      ...
      pbDisplay(_INTL("Pero los ánimos no llegaron a los oídos de nadie..."))
