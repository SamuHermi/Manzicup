  ...
  proc { |cheer, side, owner, battler, battle|
	...
	when 1
      if battler.pbOwnSide.effects[PBEffects::CheerOffense1] == 0
        ...
        battle.pbDisplay(_INTL("¡Los ánimos inspiraron a {1} a atacar con más fuerza!", battler.pbTeam(true)))
        ret = true
      end
    when 2
	  if battler.pbOwnSide.effects[PBEffects::CheerOffense2] == 0
	    battler.pbOwnSide.effects[PBEffects::CheerOffense2] = 3
		battle.pbDisplay(_INTL("¡Los ánimos inspiraron a {1} a atacar con más potencia!", battler.pbTeam(true)))
		ret = true
	  end
    when 3
	  if battler.pbOwnSide.effects[PBEffects::CheerOffense3] == 0
	    battler.pbOwnSide.effects[PBEffects::CheerOffense3] = 3
		battle.pbDisplay(_INTL("¡Los ánimos inspiraron a {1} a romper barreras!", battler.pbTeam(true)))
		ret = true
	  end
    end
    next ret
  ...
  proc { |cheer, side, owner, battler, battle|
    ...
    when 1
	  if battler.pbOwnSide.effects[PBEffects::CheerDefense1] == 0
        ...
        battle.pbDisplay(_INTL("¡Los ánimos inspiraron a {1} a resistir ataques!", battler.pbTeam(true)))
        ret = true
      end
    when 2
	  if battler.pbOwnSide.effects[PBEffects::CheerDefense2] == 0
	    battler.pbOwnSide.effects[PBEffects::CheerDefense2] = 3
		battle.pbDisplay(_INTL("¡Los ánimos inspiraron a {1} a resistir críticos y efectos!", battler.pbTeam(true)))
		ret = true
	  end
    when 3
	  if battler.pbOwnSide.effects[PBEffects::CheerDefense3] == 0
		...
		battle.pbDisplay(_INTL("¡Los ánimos inspiraron a {1} a aguantar golpes!", battler.pbTeam(true)))
		ret = true
	  end
	end
    next ret
  ...
  proc { |cheer, side, owner, battler, battle|
	...
	battle.allSameSideBattlers(battler).each do |b|
      if b.canHeal?
        ...
        battle.pbDisplay(_INTL("{1} recuperó PS.", b.pbThis))
        ret = true
      end
      if cheerLvl > 1
        if b.status != :NONE
        ...
        if b.effects[PBEffects::Confusion] > 0
          b.pbCureConfusion
          battle.pbDisplay(_INTL("{1} ya no está confuso.", b.pbThis))
          ret = true
        end
        if b.effects[PBEffects::Attract] >= 0
          b.pbCureAttract
          battle.pbDisplay(_INTL("{1} ya no está enamorado.", b.pbThis))
          ret = true
        end
        if b.effects[PBEffects::Curse]
          b.effects[PBEffects::Curse] = false
          battle.pbDisplay(_INTL("{1} se libró de la maldición.", b.pbThis))
          ret = true
        end
      end
      if cheerLvl == 3 && b.effects[PBEffects::HealBlock] == 0
		...
		battle.pbDisplay(_INTL("¡Los ánimos inspiraron a {1} a pedir un deseo!", b.pbThis(true)))
		ret = true
      end
	end
    next ret
...
Battle::Cheer::CheerEffects.add(:Counter,
  proc { |cheer, side, owner, battler, battle|
    ...
    when 1
	  ...
	  battle.pbDisplay(_INTL("¡Los ánimos revirtieron los cambios en las caractéristicas!")) if ret
    when 2
	  effects = [
      ...
      battle.pbDisplay(_INTL("Los ánimos cambiaron los efectos que habían en el campo!")) if ret
    when 3
	  battle.allSameSideBattlers(battler).each do |b|
	    next if b.effects[PBEffects::HealBlock] == 0
		b.effects[PBEffects::HealBlock] = 0
		battle.pbDisplay(_INTL("Los ánimos eliminaron Anticura a {1}!", b.pbThis(true)))
		ret = true
	  end
	  battle.allOtherSideBattlers(battler).each do |b|
	    next if b.effects[PBEffects::HealBlock] > 0
		b.effects[PBEffects::HealBlock] = 3
		battle.pbDisplay(_INTL("¡Los ánimos previenen que {1} se cure!", b.pbThis(true)))
		ret = true
	  end
    end
    next ret
...
Battle::Cheer::CheerEffects.add(:BasicRaid,
  proc { |cheer, side, owner, battler, battle|
	...
	when 2
	  ...
	  battle.pbDisplay(_INTL("La energía que rodea a {1} parece haberse debilitado...", foe.pbThis(true)))
	  battle.pbRaidChangeTurnCount(foe, count)
	  battle.pbDisplay(_INTL("¡El número de turnos ha aumentado!"))
	  next true
	when 3
	  ...
	  battle.pbDisplay(_INTL("¡El número de turnos ha aumentado!"))
	  battle.pbDisplay(_INTL("¡Ahora puedes caer debilitado más veces!"))
	  next true
	end
	next false
  }
...
Battle::Cheer::CheerEffects.add(:UltraRaid,
  proc { |cheer, side, owner, battler, battle|
	...
	battle.pbDisplayPaused(_INTL("¡Los ánimos recargaron {2} de {1}!\n¡Ahora {1} puede usar Movimientos Z!", trainerName, itemName))
    next true
  }
)
...
Battle::Cheer::CheerEffects.add(:MaxRaid,
  proc { |cheer, side, owner, battler, battle|
	...
	battle.pbDisplayPaused(_INTL("¡Los ánimos recargaron {2} de {1}!\n¡Ahora {1} puede usar el Dinamax!", trainerName, itemName))
    next true
  }
)
...
Battle::Cheer::CheerEffects.add(:TeraRaid,
  proc { |cheer, side, owner, battler, battle|
	...
	battle.pbDisplayPaused(_INTL("¡Los ánimos recargaron {2} de {1}!\n¡Ahora {1} puede usar la Terastalización!", trainerName, itemName))
    next true
  }
)