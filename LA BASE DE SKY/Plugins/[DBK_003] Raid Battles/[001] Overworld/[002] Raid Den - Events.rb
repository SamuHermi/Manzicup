...
def pbRaidDenReset(interp, this_event)
  if $DEBUG && Input.press?(Input::CTRL)
    pbMessage(_INTL("Un nuevo Pokémon ha aparecido!"))
    interp.setVariable(nil)
	this_event.turn_up
	return true
  else
    item = GameData::Item.get(:RAIDBAIT)
    pbMessage(_INTL("No parece que haya nada..."))
    if pbConfirmMessage(_INTL("¿Quieres tirar un {1} para atraer a un Pokémon?", item.portion_name))
      if $bag.has?(item.id)
        pbMessage(_INTL("Tiraste un {1}!", item.portion_name))
        $bag.remove(item.id)
        interp.setVariable(nil)
		this_event.turn_up
		return true
      else
        pbMessage(_INTL("No tienes ningún {1}...", item.portion_name_plural))
      end
    end
  end
  return false
