...
class RaidScene
  ...
  def pbSavingPrompt(pkmn, rules)
    ...
    this_event.turn_up
    pbMessage(_INTL("Hechas un vistazo al {1}...", den_name))
    return true
  end
  ...
  def pbStartScene(pkmn, rules)
	...
	@sprites["cursor"] = IconSprite.new(418, 159)
	...
	@pkmn.types.each_with_index do |type, i|
      ...
      type_x = (i == 0) ? 24 : 124
      imagePos.push([typepath, type_x, 124, 0, type_number * 32, 96, 32])
    end
    #---------------------------------------------------------------------------
    # Additional icons
    #---------------------------------------------------------------------------
	...
	when :Tera   # Tera type icon
      ...
      imagePos.push([Settings::TERASTAL_GRAPHICS_PATH + "tera_types", xpos + 60, 120, 0, type_number * 32, 32, 32])
	  xpos -= 34
	end
	...
	if @rules[:partner]
	  ...
	  [$player, $PokemonGlobal.partner].each_with_index do |tr, i|
		...
		imagePos.push([trainer_sprite, party_x - 36 + (offset * i), 252, 0, 0, 48, 48],
		              [@path + "#{@rules[:style]}/icon_party", party_x + (offset * i), 280])
        ...
        @sprites["partyicon_#{i}"].y = 310
	  end
	else
	  party_x = 500 - (18 * @rules[:size])
      @rules[:size].times { |i| imagePos.push([@path + "#{@rules[:style]}/icon_party", party_x + (56 * i), 280]) }
    ...
    party_config = (@rules[:size] > 1) ? _INTL("Cambiar equipo") : _INTL("Cambiar Pokémon")
    textPos = [
      [raid_name.upcase,            97,  24, :center, BASE, SHADOW, :outline],
      [_INTL("Empezar incursión"), 523, 168, :center, BASE, SHADOW, :outline],
      [_INTL("Salir"),             523, 202, :center, BASE, SHADOW, :outline],
      [party_config,               523, 235, :center, BASE, SHADOW, :outline]
    ]
	showTurns = @rules[:turn_count] && @rules[:turn_count] > 0
	showKOs = @rules[:ko_count] && @rules[:ko_count] > 0
	ko_text = (@rules[:ko_count] == 1) ? "desmayo" : "desmayos"
	if showTurns
	  if showKOs
	    battleText = "La pelea acaba en #{@rules[:turn_count]} turnos o tras #{@rules[:ko_count]} #{ko_text}."
	  else
	    battleText = "La pelea acaba en #{@rules[:turn_count]} turnos."
	  end
	elsif showKOs
	  battleText = "La pelea acaba tras #{@rules[:ko_count]} #{ko_text}."
	else
	  battleText = "¡Derrota al Pokémon!"
	end
    #---------------------------------------------------------------------------
    pbDrawImagePositions(@overlay, imagePos)
    pbDrawTextPositions(@overlay, textPos)
	  drawTextEx(@overlay, 170, 279, 226, 2, _INTL(battleText), BASE, SHADOW)
  ...
  def pbRaidEntry
    ...
    loop do
      ...
      if Input.trigger?(Input::UP)
        ...
        if pbConfirmMessage(_INTL("¿Quieres acabar la incursión?"))
        ...
        when 0 # Begin Raid
          @sprites["cursor"].visible = false
		  party_display = (@rules[:size] > 1) ? "equipo" : "Pokémon"
          if pbConfirmMessage(_INTL("Entrar a la incursión con el {1} seleccionado?", party_display))
  ...
  def pbRaidRewardsScreen(outcome)
	...
	@rules[:rank].times { |i| imagePos.push([@path + "icon_star", Graphics.width - 58 - i * 38, 88]) }
	#---------------------------------------------------------------------------
    # Additional icons
    #---------------------------------------------------------------------------
	if @pkmn.shiny?
    ...
    case outcome
    when 1
      result  = _INTL("¡Has derrotado a {1}!", @pkmn.name)
    when 4
      result  = _INTL("¡Has capturado a {1}!", @pkmn.name)
	  level   = _INTL("Nv. {1}", @pkmn.level)
      ...
      result = _INTL("¡Has perdido contra {1}!", @pkmn.name)
      textPos.push([_INTL("Sin recompensa."), 367, 180, :center, BASE, SHADOW])
    end
    textPos.push(
      [result,        377,  24, :center, BASE, SHADOW, :outline],
      [level,          32,  78, :left,   BASE, SHADOW, :outline],
      [ability,        32, 290, :left,   BASE, SHADOW, :outline],
	  [_INTL("Sig."), 438, 290, :center, BASE, SHADOW, :outline]
    )
    pbDrawTextPositions(@overlay, textPos)
    #---------------------------------------------------------------------------
    # Screen controls
