class Battle::Scene::PokemonDataBox
  ...
  def draw_type_icons
    ...
    if extra_type
      type_number = GameData::Type.get(extra_type).icon_position
      type_rect = Rect.new(0, type_number * height, width, height)
      y_position = @battler.types.size * (height + separacion_extra) / 4
      @types_sprite.bitmap.blt(0, y_position, @types_bitmap.bitmap, type_rect)
      @types_sprite.z = 99_999_999
    end
