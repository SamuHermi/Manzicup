...
class BannerReward < Sprite
  def initialize(id, reward, stars, viewport = nil)
    ...
    elsif @rewardSprite.bitmap.height <= 89
  ...
  def x=(value)
    super(value)
    @rewardSprite.x = value + bitmap.width / 2 - ox
    @starsSprite.x = value + 36 + @starsSprite.ox - ox
  end

  def y=(value)
    super(value)
    y = if @animated_sprites
          (@id != 1 ? 30 : 5) # Corrección de altura sprite pokemon animados
        else
          (@id != 1 ? -5 : -25) # Corrección de altura sprite pokemon estáticos
        end
    @rewardSprite.y = value + y + @rewardSprite.oy - oy
    y2 = (@id != 1 ? 120 : 90) # Corrección de altura sprite estrellas
    @starsSprite.y = value + y2 + @starsSprite.oy - oy
  end

  def zoom_x=(value)
    super(value)
    x = if @animated_sprites # Ajuste al zoom x del premio
          0
        else
          -1
        end
  ...
  def zoom_y=(value)
    super(value)
    y = if @animated_sprites # Ajuste al zoom y del premio
          0
        else
          -1
        end
...
class GachaScene
  def initialize(banners)
    $player.money ||= 0 unless $player.money
    ...
    for i in 0...@banners.length
      @sprites["banner#{i}"] =
        BannerSprite.new(@banners[i].bg, @banners[i].rewards, @banners[i].stars, @viewport, @banners[i].cost)
  ...
  def refresh
    ...
    @sprites['button1'].bitmap = if @sel == 0
                                   Bitmap.new('Graphics/UI/Gacha/Boton Chiquito Sel')
                                 else
                                   Bitmap.new('Graphics/UI/Gacha/Boton Chiquito')
                                 end
    ...
    @sprites['button2'].bitmap = if @sel == 2
                                   Bitmap.new('Graphics/UI/Gacha/Boton Sel')
                                 else
                                   Bitmap.new('Graphics/UI/Gacha/Boton')
                                 end
    ...
    @sprites['button3'].bitmap = if @sel == 3
                                   Bitmap.new('Graphics/UI/Gacha/Boton Sel')
                                 else
                                   Bitmap.new('Graphics/UI/Gacha/Boton')
                                 end
    ...
    @sprites['button4'].bitmap = if @sel == 1
                                   Bitmap.new('Graphics/UI/Gacha/Boton Chiquito Sel')
                                 else
                                   Bitmap.new('Graphics/UI/Gacha/Boton Chiquito')
                                 end
    ...
    @sprites['bannerSel'].visible = @sel == 4

    @sprites['leftArrow'].opacity = if @banner_sel == 0
                                      50
                                    else
                                      255
                                    end

    @sprites['rightArrow'].opacity = if @banner_sel == @banners.length - 1
                                       50
                                     else
                                       255
                                     end
    ...
    drawTextEx(@sprites['text'].bitmap, 100, 70, 900, 2, @banners[@banner_sel].name, Color.new(88, 88, 88),
    ...
    pbDrawShadowText(@sprites['costmult'].bitmap, 9, 12, 75, 48, 'x' + (@banners[@banner_sel].cost * 10).to_s, base,
                     shadow, 1)
  end
  ...
  def summaryWindow(width, interpad)
    ...
    colortag = ''
    colortag = if ($game_message && $game_message.background > 0) ||
                  ($game_system && $game_system.respond_to?('message_frame') &&
                   $game_system.message_frame != 0)
                 getSkinColor(window.windowskin, 0, true)
               else
                 getSkinColor(window.windowskin, 0, isDarkSkin)
               end
    text = colortag + text
    ...
    loop do
      window.oy -= 10 if Input.press?(Input::UP) && window.oy != 0
      window.oy += 10 if Input.press?(Input::DOWN) && window.oy < canvas.height - 200
      ...
      if Input.trigger?(Input::C)
        ...
        when 2 # Single
          if $player.money >= @banners[@banner_sel].cost
            if pbMessage('¿Quieres hacer un single? SE GUARDARÁ LA PARTIDA', %w[Sí No]) == 0
        ...
        when 3 # Multi
          if $player.money >= @banners[@banner_sel].cost * 10
            if pbMessage('¿Quieres hacer un multi? SE GUARDARÁ LA PARTIDA', %w[Sí No]) == 0
