...
module Roulette
  RED   = [MessageConfig::NEREA_TEXT_COLOR, MessageConfig::NEREA_TEXT_SHADOW_COLOR]
  ...
  class Scene
    def startScene(wager, backgroundColor, usedIconColor)
      ...
      @sprites["table"]=IconSprite.new(0,Settings::SCREEN_WIDTH/2,@viewport)
      ...
      @displayedCredits = $player.battle_points
      @exit=false
      @tableBallPosArray = []
    ...
    def refreshBottomElementsPos
      ...
      @sprites["table"].y=Graphics.height*3/4-@sprites["table"].bitmap.height-10
    ...
    def processResults
      ...
      if wins
        if @cursor.multiplier==12
          ...
          displayMessage(_INTL(
            "\\me[{1}]¡Has acertado!\\wtnp[30]", Bridge.getAudioName("Slots win")
          ))
        end
        $player.battle_points += @wager*@cursor.multiplier
        displayMessage(_INTL("Has ganado {1} Manzicoins!",@wager*@cursor.multiplier))
      else  
        pbPlayBuzzerSE()
        displayMessage(_INTL("¡Mala suerte!"))
      end
      @playedBalls[@result]=true
      if Roulette.count(@playedBalls,true)==ROUNDS # Clear
        if ROUNDS!=1
          displayMessage(_INTL("Se va a reiniciar el tablero."))
        end
        @roulette.clearBalls
        @playedBalls.clear
      ...
      drawMultiplier
      if confirmMessage(_INTL("¿Seguir jugando?"))
        if $player.battle_points<@wager
          displayMessage(_INTL("¡No tienes suficientes monedas para jugar!"))
          @exit=true 
        end
      else  
        @exit=true 
  ...
  def self.start(wager=1, backgroundAndIconColor=nil)
    ...
    goldwindow.y = 0
    goldwindow.z = 99999
    if Bridge.confirmMessage(_INTL(
      "Vas a apostar {1}. ¿Quieres continuar?", wager
    ))
      if $player.battle_points<wager
        Bridge.message(_INTL("¡No tienes suficientes Manzicoins!"))
      elsif $player.battle_points==Bridge.maxCoins
