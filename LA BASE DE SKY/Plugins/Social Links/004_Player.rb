...
class Player < Trainer
            ...
            val.each do |value|
                ...
                if bond < key && bond_effects[value[0]] && bond_effects[value[0]].include?(array)
                    ...
                    if SocialLinkSettings::SHOW_BOND_EFFECT_MESSAGE && !silent
                        ...
                        when :EXP
                            if array[1] > 1
                                pbMessage(_INTL("¡Tus Pokémon de tipo {1} ahora ganarán más experiencia!",GameData::Type.get(array[0]).name))
                            end
                        when :Shiny
                            if array[1] >= 1
                                pbMessage(_INTL("¡Los Pokémon de tipo{1} ahora tienen más probabilidad de ser Shiny!",GameData::Type.get(array[0]).name))
                            end 
                        when :MOVE
                            if array[1] >= 1
                                pbMessage(_INTL("¡La potencia de los Pokémon de tipo {1} ha aumentado!",GameData::Type.get(array[0]).name))
                            end                             
                        end
                    end
                end
    ...
    class SocialLinks
        ...
        def pbMaxLinkBond(profile_id,silent:true)
            profile_id = pbSanitizeProfileID(profile_id)
            return false if profile_id.nil?
            return false if !@links[profile_id]
            max_bond = @links[profile_id].bond_max
            @links[profile_id].bond = max_bond
            $player.refresh_bond_effects(profile_id, @links[profile_id].bond, silent) if GameData::SocialLinkProfile.get(profile_id)&.bond_effects
        end
