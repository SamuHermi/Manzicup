  ...
  def refresh
    ...
    2.times do |i|
      ...
      if @sprites["shadow_#{i}"].bitmap
        ...
        @sprites["shadow_#{i}"].y -= @sprites["pokemon_#{i}"].bitmap.height / 4 if (i == 0 && !Settings::INVERTIR_SOMBRA_JUGADOR)
  ...
  def refresh
    ...
    2.times do |i|
      ...
      if @sprites["shadow_#{i}"].bitmap
        ...
        @sprites["shadow_#{i}"].y -= 25 if (i == 0 && !Settings::INVERTIR_SOMBRA_JUGADOR)
      end
      @sprites["shadow_#{i}"].visible = metrics_data.shows_shadow?
    end
  end
