...
def pbDisplayIVRatings(pokemon, overlay, xpos, ypos, horizontal = false)
  ...
  GameData::Stat.each_main do |s|
    ...
    icon = case stat
           when maxIV     then 5  # 31 IV
           when maxIV - 1 then 4  # 30 IV
           when 0         then 0  #  0 IV
           else
...
MenuHandlers.add(:pokemon_debug_menu, :set_shiny_leaf, {
                   ...
                   'effect' => proc { |pkmn, pkmnid, heldpoke, settingUpBattle, screen|
                     cmd = 0
                     loop do
                       msg = [_INTL('Tiene corona brillante.'),
                              _INTL("Número de hojas brillantes: x#{pkmn.shiny_leaf}.")][pkmn.shiny_crown? ? 0 : 1]
                       ...
                       when 0   # Set Leaf
                         ...
                         leafcount = pbMessageChooseNumber(
                           _INTL('Indica el contador de hojas de {1} (máx. 6).', pkmn.name), params
                         ) { screen.pbUpdate }
