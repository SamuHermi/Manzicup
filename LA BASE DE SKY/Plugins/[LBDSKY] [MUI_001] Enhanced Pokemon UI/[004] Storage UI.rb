  ...
  def pbUpdateOverlay(selection, party = nil)
    ...
    pbDrawTextPositions(
      ...
      [[_INTL('Equipo: {1}', begin
        ...
        0
      end), 270, 392, :center, buttonbase, buttonshadow, :outline],
    ...
    unless pokemon.egg?
      ...
      if pokemon.level < LevelCapsEX.level_cap
        ...
        textstrings.push(['Nv. ' + pokemon.level.to_s, 110, 260, :left, Color.new(248, 192, 0), Color.new(144, 104, 0)])
      end

      textstrings.push([pokemon.nature.name.to_s, 10, 260, :left, base, shadow])
      if pokemon.ability
        textstrings.push([pokemon.ability.name, 90, 351, :center, base, shadow])
      else
        textstrings.push([_INTL('Sin habilidad'), 90, 351, :center, nonbase, nonshadow])
      end
      if pokemon.item
        textstrings.push([pokemon.item.name, 90, 385, :center, base, shadow])
      else
        textstrings.push([_INTL('Sin objeto'), 90, 385, :center, nonbase, nonshadow])
      end
      pbDrawImagePositions(plugin_overlay, [['Graphics/Pictures/shiny', 134, 16]]) if pokemon.shiny?
