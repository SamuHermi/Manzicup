...
class PokemonSummary_Scene
  ...
  def drawPage(page)
    ...
    return unless Settings::SUMMARY_SHINY_LEAF
  ...
  def drawPageOne
    ...
    return unless Settings::SUMMARY_HAPPINESS_METER
  ...
  def pbPageCustomUse(page_id)
    if (page_id == :page_skills) && defined?(Settings::DISPLAY_ENHANCED_STATS) && Settings::DISPLAY_ENHANCED_STATS
      @statToggle = !@statToggle
      drawPage(:page_skills)
      pbPlayDecisionSE
      return true
    end
    enhanced_pbPageCustomUse(page_id)
  end
  ...
  def pbPageCustomOption(cmd)
    ...
    enhanced_pbPageCustomOption(cmd)
  end
  ...
  def pbLegacyMenu
    ...
    loop do
      ...
      if index > 0
        imagepos.push([path + 'arrows_legacy', LEGACY_ARROW_RIGHT_X, ypos + LEGACY_ARROW_RIGHT_Y_OFFSET, 0, 0, 32,
                       32])
      end
  ...
  def drawEnhancedStats
    ...
    GameData::Stat.each_main do |s|
      case s.id
      when :HP then xpos = ENHANCED_HP_X
                    ypos = STATS_HP_Y
                    align = :center
      else xpos = STATS_DEFAULT_X
           ypos = STATS_FIRST_Y + (STATS_Y_SPACING * index)
           align = :left
      end
      name = if s.id == :SPECIAL_ATTACK
               _INTL('Atq. Esp.')
             else
               s.id == :SPECIAL_DEFENSE ? _INTL('Def. Esp.') : s.name
             end
      statshadow = shadow
    ...
    textpos.push(
      ...
      [format('%d  |  %d', ev_total, iv_total), STATS_TOTAL_VALUE_X, STATS_TOTAL_Y, :center, base2, shadow2],
      [_INTL('EVs restantes:'), STATS_REMAIN_LABEL_X, STATS_REMAIN_Y, :left, base2, shadow2],
      [format('%d/%d', Pokemon::EV_LIMIT - ev_total, Pokemon::EV_LIMIT), STATS_REMAIN_VALUE_X, STATS_REMAIN_Y,
       :center, base2, shadow2],
