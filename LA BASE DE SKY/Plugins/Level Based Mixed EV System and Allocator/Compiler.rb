module Compiler
  ...
  def validate_compiled_trainer(hash)
    ...
    raise _INTL("Trainer with ID {1} has no Pokémon.\n{2}", hash[:id], FileLineData.linereport) if hash[:pokemon].empty?
    ...
    hash[:pokemon].each do |pkmn|
      ...
      if pkmn[:ev]
        ...
        pkmn[:ev] = ev_hash
        Console.echo_li("\nEVs for #{pkmn[:species]}: #{pkmn[:ev].inspect}\n")
        Console.echo_li("Total EVs for #{pkmn[:species]}: #{ev_total}\n")
        if ev_total > 512
      ...
      if pkmn[:happiness] && (pkmn[:happiness] > 255)
        raise _INTL("Bad happiness: {1} (must be 0-255).\n{2}", pkmn[:happiness], FileLineData.linereport)
      end
      # Ensure valid Poké Ball
      next unless pkmn[:poke_ball]
      unless GameData::Item.get(pkmn[:poke_ball]).is_poke_ball?
