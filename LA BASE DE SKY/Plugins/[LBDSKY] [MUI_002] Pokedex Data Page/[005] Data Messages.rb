...
class PokemonPokedexInfo_Scene
  ...
  def pbDrawDataNotes(cursor = nil)
    ...
    when :ability
      ...
      text << if $player.owned?(@species)
                _INTL('Ver todas las habilidades de esta especie.')
              else
                _INTL('Desconocido.')
              end
    when :moves
      ...
      text << if $player.owned?(@species)
                _INTL('Ver todos los movimientos que esta especie puede aprender.')
              else
                _INTL('Desconocido.')
              end
    end
  ...
  def pbDataTextStats(path, species, overlay, s2 = nil)
    ...
    if owned
      ...
      stats_order = [%i[HP SPECIAL_ATTACK], %i[ATTACK SPECIAL_DEFENSE], %i[DEFENSE SPEED]]
      stats_order.each_with_index do |st, i|
        names = values = ''
        st.each_with_index do |s, j|
          stat = if s == :SPECIAL_ATTACK
                   _INTL('At. Esp.')
                 else
                   s == :SPECIAL_DEFENSE ? _INTL('Def. Esp.') : GameData::Stat.get(s).name
                 end
          nt = s2 && s2[s] == s1[s] ? t[2] : t[0]
          names += nt + _INTL('{1}', stat)
          if !s2 && @api_data
            color = if s1[s] > @api_data['stats'][s]
                      t[3]
                    elsif s1[s] < @api_data['stats'][s]
                      t[1]
                    else
                      t[0]
                    end
  ...
  def pbDataTextHabitat(path, species, overlay, s2 = nil)
    ...
    if owned
      ...
      text << case habitat.id
              when :Grassland
                _INTL('Esta especie se puede encontrar correteando por zonas de {1}{2}{3}.', nt, name, t[0])
              when :Forest
                _INTL('Esta especie se puede encontrar en zonas densas de {1}{2}{3}.', nt, name, t[0])
              when :WatersEdge
                _INTL('Esta especie se puede encontrar cerca de zonas de {1}{2}{3}.', nt, name, t[0])
              when :Sea
                _INTL('Esta especie se puede encontrar en zonas de {1}{2}{3}.', nt, name, t[0])
              when :Cave
                _INTL('Esta especie se puede encontrar en zonas de {1}{2}{3}.', nt, name, t[0])
              when :Mountain
                _INTL('Esta especie se puede encontrar en zonas escarpadas de {1}{2}{3}.', nt, name, t[0])
              when :RoughTerrain
                _INTL('Esta especie se puede encontrar en zonas de {1}{2}{3}.', nt, name, t[0])
              when :Urban
                _INTL('Esta especie se puede encontrar cerca de estructuras creadas por humanos o de {1}{2}{3}.', nt,
                      name, t[0])
              when :Rare
                _INTL('Esta especie se puede encontrar en lugares bastante {1}{2}{3}.', nt, name, t[0])
              else
                _INTL('Esta especie se puede encontrar en sitios desconocidos.')
              end
  ...
  def pbDataTextShape(path, species, overlay, s2 = nil)
    ...
    text << case shape.id
            when :Head, :Serpentine, :Finned, :HeadArms, :HeadBase, :Winged, :Multiped, :MultiBody, :MultiWinged
              _INTL('Tiene forma de {1}{2}{3}.', nt, name, t[0])
            when :Bipedal, :BipedalTail, :HeadLegs, :Quadruped, :Insectoid
              _INTL('Tiene forma {1}{2}{3}.', nt, name, t[0])
            else
              _INTL('La forma no puede ser clasificada.')
            end
  ...
  def pbDataTextEggGroup(path, species, overlay, s2 = nil)
    ...
    if owned
      ...
      if groups.include?(:None)
        ...
        text << if size_groups_sky == 2
                  _INTL('compatible con los grupos ')
                else
                  _INTL('compatible con el grupo ')
                end
        groups.each_with_index do |group, i|
  ...
  def pbDataTextFamily(path, species, overlay, inMenu = false)
    ...
    if nil_or_empty?(text)
      ...
      #-------------------------------------------------------------------------
      if %i[PICHU_2 FLOETTE_5 GIMMIGHOUL_1].include?(species.id) ||
      ...
      if prevo == species.species
        ...
        if family_ids.empty?          # Species doesn't evolve.
          ...
          family_ids.each_with_index do |fam, i|
            ...
            text << t[1] + name
            next unless i < family_ids.length - 1

            text << if fam == GameData::Species.get(family_ids[i + 1]).get_previous_species
                      t[0] + '=> ' # Shows evolution pathway.
                    else
                      t[0] + ', ' # Shows a new pathway.
                    end
          end
          text << t[0] + '.'
        end
      ...
      #-------------------------------------------------------------------------
        ...
        if species.species == :ALCREMIE
          ...
          evolutions.each do |evo|
            ...
            if species.species == :URSHIFU && evo[1] == :Item && (evo[2] != %i[SCROLLOFDARKNESS
                                                                               SCROLLOFWATERS][species.form])
              next
            end
            ...
            unless nil_or_empty?(text)
              if count == 2
                text << _INTL(' o ')
              elsif count > 2
                text << if index == count - 1
                          _INTL(' o ')
                        else
                          ', '
                        end
        ...
        if species.form_name
          Settings::REGIONAL_NAMES.each do |region|
            next unless species.form_name.include?(region)
        ...
        if nil_or_empty?(heading)
          ...
          heading = if inMenu && evos[-1][0] == prevo && evos.any? { |evo| evo[0] != prevo && evo[1] == :None }
                      t[0] + _INTL("Método evolutivo (etapa final)\n")
                    else
                      t[0] + _INTL("Método evolutivo\n")
                    end
        end
        text = heading + text
      end
  ...
  def pbDrawSpecialFormText(species)
    ...
    if special_form
      ...
      spname = if nil_or_empty?(form_name)
                 base_data.name
               elsif form_name.include?(base_data.name)
                 form_name
               else
                 form_name + ' ' + base_data.name
               end
      case special_form
      #-------------------------------------------------------------------------
      # Mega forms
      ...
      #-------------------------------------------------------------------------
      when :gmax
        if species.has_flag?('AllFormsShareGmax') || species.species == :TOXTRICITY
          spname = _INTL('cualquier forma de {1}',
                         base_data.name)
        end
  ...
  def pbDataTextAbilitySource(path, species, overlay, ability)
    ...
    if species.abilities.include?(ability)
      ...
      when 1 # Species only has one base ability.
        text << if species.hidden_abilities.empty? ||
                   species.mega_stone || species.mega_move
                  _INTL('la {1}única{2} habilidad de esta especie.', t[1], t[0])
                else
                  _INTL('la habilidad {1}base{2} de esta especie.', t[1], t[0])
                end
      when 2 # Species has two base abilities.
        text << if species.abilities[0] == ability
                  _INTL('la habilidad {1}primaria{2} de esta especie.', t[1], t[0])
                else
                  _INTL('la habilidad {1}secundaria{2} de esta especie.', t[1], t[0])
                end
      end
    #---------------------------------------------------------------------------
    # Hidden abilities.
  ...
  def pbDataTextMoveSource(path, species, overlay)
    ...
    species.moves.each do |m|
      ...
      method = case m[0]
               when -1 then _INTL('en el {1}recuerda movimientos{2}', t[1], t[0]) # Gen 9 move relearning.
               when 0  then _INTL('al {1}evolucionar{2}', t[1], t[0]) # Evolution move.
               else _INTL('a {1}nivel {2}{3}', t[1], m[0], t[0]) # Level-up move.
               end
      methods.push(method)
      break
    end
