...
module Settings
  ...
  #-----------------------------------------------------------------------------
  REGIONAL_NAMES = %w[Alola Galar Hisui Paldea]
end
...
class PokemonPokedexInfo_Scene
  ...
  def pbStartScene(*args)
    ...
    @maxmoves = GameData::Move.get_generic_dynamax_moves if PluginManager.installed?('[DBK] Dynamax')
  ...
  def pbPageCustomUse(page_id)
    ...
    modular_pbPageCustomUse(page_id)
  end
  ...
  def pbResetFamilyIcons
    3.times do |i|
      next unless @sprites["familyicon#{i}"]
  ...
  def gender_difference?(form)
    ...
    [_INTL('Macho'), _INTL('Hembra')].include?(species_data.form_name)
  end
