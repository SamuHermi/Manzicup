...
GameData::Habitat.each do |habitat|
  habitat.icon_position = case habitat.id
                          when :None         then 0
                          when :Grassland    then 1
                          when :Forest       then 2
                          when :WatersEdge   then 3
                          when :Sea          then 4
                          when :Cave         then 5
                          when :Mountain     then 6
                          when :RoughTerrain then 7
                          when :Urban        then 8
                          when :Rare         then 9
                          else                    0
                          end
...
module GameData
  class EggGroup
    attr_accessor :alt_name, :icon_position
  ...
  class Evolution
    ...
    def description(species, evo, param = nil, full = true, form = false, c = [])
      ...
      if GameData::Species.exists?(species)
        ...
        full_name = if form && form_name && !form_name.include?(species_data.name)
                      _INTL('{2} {3}{1}', prefix, species_data.name, species_data.form_name)
                    else
                      _INTL('{2}{1}', prefix, species_data.name)
                    end
      else
        full_name = _INTL('????')
      end
      full_name = c[1] + full_name + c[0] unless c.empty?
      #-------------------------------------------------------------------------
      ...
      when Symbol
        ...
        prefix = ''
        if %i[Type Item Species].include?(@parameter)
          prefix = '' # (par.starts_with_vowel?) ? "an " : "a "
        end
        param = c[2] + par + c[0] unless c.empty?
      ...
      when Integer
        ...
        if param_name
          param_name = c[2] + param_name + c[0] unless c.empty?
...
module GameData
  class Species
    ...
    def display_species?(dexlist, species, special = false, skip_owned_check = false)
      return false unless dexlist.any? { |dex| dex[:species] == @species }
      return false if !$player.owned?(@species) && !skip_owned_check
      return false if @species == species.species && @form == species.form

      if @form > 0 && !Settings::DEX_SHOWS_ALL_FORMS && !($player.pokedex.seen_form?(@species, 0, @form) ||
      ...
      return false if %i[PICHU_2 FLOETTE_5 GIMMIGHOUL_1].include?(@id)
    ...
    def is_regional_form?
      if @form > 0 && form_name
        return false if form_name.include?(_INTL('Forma Daruma'))

        Settings::REGIONAL_NAMES.each do |region|
          return true if form_name.include?(region)
        end
      end
      false
    end
    ...
    def get_tutor_moves
      ...
      when :KYUREM_1    then moves = %i[ICEBURN FUSIONFLARE]
      when :KYUREM_2    then moves = %i[FREEZESHOCK FUSIONBOLT]
      ...
      return tutor_moves.sort unless moves

      (tutor_moves + moves).sort
    end
    ...
    def get_inherited_moves
      moves = case get_baby_species
              when :PICHU then [:VOLTTACKLE]
              else             []
              end
      (get_egg_moves + moves).sort
    end
