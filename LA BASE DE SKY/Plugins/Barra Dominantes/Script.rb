...
def draw_level
  pbDrawImagePositions(self.bitmap, [[_INTL("Graphics/UI/Battle/overlay_lv"), @spriteBaseX + 128, 16]])
  pbDrawNumber(@battler.level, self.bitmap, @spriteBaseX + 154, 16)
end
...
def draw_special_form_icon
  if @battler.mega?
    pbDrawImagePositions(self.bitmap, [["Graphics/UI/Battle/icon_mega", @spriteBaseX - 30, 54]])
    ...
    primalX = (@battler.opposes?) ? 205 : -25
    pbDrawImagePositions(self.bitmap, [[filename, @spriteBaseX - primalX, 54]]) if filename
  end
end
...
class Battle::Scene::PokemonDataBox < Sprite
   def initializeOtherGraphics(viewport)
    ...
    @types_x = (@battler.opposes?(0)) ? 224 : -44 # 1era variable oponente 2da variable jugador
    ...
    separacion_extra = 1 # Ajusta este valor según necesites
    ...
    @types_y = -total_height + 68 if @battler.types.size > 0
    #Console.echo_li("@types_y = " + @types_y.to_s)
 ...
 def x=(value)
  ...
  def draw_type_icons
    ...
    width  = @types_bitmap.width #+ 2*separacion_extra
    ...
    if @battler.opposes?(0) && $game_switches[65]
      ...
      if @battler.types.size == 1
        type = @battler.types[0]
        type_number = GameData::Type.get(type).icon_position
