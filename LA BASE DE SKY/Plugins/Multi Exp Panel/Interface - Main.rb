...
class Swdfm_Exp_Screen
  ...
  def update_bars
    for i in 0...$player.party.size
      ...
      if lvl > @levels[i]
        @levels[i] = if lvl > $game_variables[LevelCapsEX::LEVEL_CAP_VARIABLE]
                       $game_variables[LevelCapsEX::LEVEL_CAP_VARIABLE]
                     else
                       lvl
                     end
        redraw_level(i)
      ...
      dispose_if_there("exp_#{i}") if @elapsed == (ANNOUCE_TIME * 40) || @elapsed == @total_frames
      bmp = @sprites["bar_1_#{i}"].bitmap
      bmp = if perc
              Swdfm_Bitmap.colour(bmp, @exp_color, 0, 0, (bmp.width * perc / 100).floor, bmp.height)
            else
              Swdfm_Bitmap.colour(bmp, @exp_color, 0, 0, 0, 0)
            end
  ...
  def draw_party
    ...
    for i in 0...$player.party.length
      ...
      bmp = Swdfm_Bitmap.text("Nv. #{$player.party[i].level}", hash, @sprites["level_#{i}"].bitmap)
  ...
  def redraw_level(i)
    ...
    bmp = Swdfm_Bitmap.text("Nv. #{@levels[i]}", hash, @sprites["level_#{i}"].bitmap)
  end
end
