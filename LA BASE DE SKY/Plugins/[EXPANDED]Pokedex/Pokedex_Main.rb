...
class PokemonPokedex_Scene
  ...
  def pbStartScene
    ...
    @typebitmap         = AnimatedBitmap.new("Graphics/UI/Pokedex" + _INTL("/icon_types"))
...
#    end
  ...
  def pbRefresh
    ...
    textpos = [
      [dexname, (Graphics.width / 2), 10, :center, baseWhite, shadowWhite],
      [_INTL("A/S: Desplazarse"), 20, 410, :left, baseWhite, shadowWhite],
      [_INTL("D: Buscar"), (Graphics.width / 2) + 20, 410, :center, baseWhite, shadowWhite],
      [_INTL("Z: Filtro"), Graphics.width-20, 410, :right, baseWhite, shadowWhite]
    ]
    textpos.push([GameData::Species.get(iconspecies).name, 148, 58, :center, baseWhite, shadowWhite]) if (Settings::SHOW_SILHOUETTES_IN_DEX && $player.seen?(iconspecies)) || (!Settings::SHOW_SILHOUETTES_IN_DEX && iconspecies)
    if @searchResults
      ...
      textpos.push([_INTL("Avistados:"), 48, 308, :left, baseWhite, shadowWhite])
      # Posicionamiento en base al idioma
        pos_x_aux = 160
      textpos.push([$player.pokedex.seen_count(pbGetPokedexRegion).to_s, pos_x_aux, 308, :left, baseWhite, shadowWhite])
      textpos.push([_INTL("Capturados:"), 48, 351, :left, baseWhite, shadowWhite])
      # Posicionamiento en base al idioma
      
        pos_x_aux = 176
      textpos.push([$player.pokedex.owned_count(pbGetPokedexRegion).to_s, pos_x_aux, 351, :left, baseWhite, shadowWhite])
    end
    # Draw all text
